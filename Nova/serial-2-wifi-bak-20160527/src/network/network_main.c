...
static INT32 NETWORK_handle_info(PSERIAL2WIFI_MSG_DATA pMsgData)
{
	...
	serial2wifimsgBuffFree(pMsgData);
	...
	else if(TCP == g_NetWork.type)
	{
		if(CLIENT == g_NetWork.mode || CLIENT_SERVER == g_NetWork.mode)
		{
			if(g_NetWork.client_fd >= 0)
			{
				...
				AP_LOG(AP_LOG_INFO, LOG_MOUDLES_NETWORK, "send %d\n", g_NetWork.client_fd);
			}
		}
...
void *NETWORK_rx_Client_pthread(void* args)
{
	...
	else if(TCP == g_NetWork.type)
	{
		while(1)
		{
			...
			if(count <= 0)
			{
				AP_LOG(AP_LOG_INFO, LOG_MOUDLES_NETWORK, "recv err = %d, errno=%d\n", count,errno);
				OSAL_TaskDelay(1000);
				continue;
			}
...
TX_RESTART:
	...
	if(CLIENT == g_NetWork.mode || CLIENT_SERVER == g_NetWork.mode)
	{
		...
		else if(TCP == g_NetWork.type)
		{
			...
			AP_LOG(AP_LOG_INFO, LOG_MOUDLES_NETWORK, "create tcp client fd = %d\n", g_NetWork.client_fd);
			...
			AP_LOG(AP_LOG_INFO, LOG_MOUDLES_NETWORK, "connect %d\n", g_NetWork.client_fd);
		}
	}
...
void *UART_tx_pthread(void* args)
{
	...
	int retcode = OSAL_QueueGetIdByName(&qUartTx, QUEUE_UART_TX_NAME);
	if(retcode != OSAL_SUCCESS)
	{
		AP_LOG(AP_LOG_ERR,LOG_MOUDLES_SERIAL,"OSAL_QueueGetIdByName return =%d\n", retcode);
		//printf("serial2wifimsgBuffAlloc start\n");
		return NULL;
	}
	...
	{
		printf("serial2wifimsgBuffAlloc start\n");
		pMsgData = serial2wifimsgBuffAlloc();
		if(pMsgData == NULL)
		{
			printf("serial2wifimsgBuffAlloc fail\n");
			OSAL_TaskDelay(1000);
			continue;
		}

		printf("serial2wifimsgBuffAlloc ok\n");

		pMsgData->datalen = sizeof("abc89248320483024832");
		
		memcpy(pMsgData->data, "abc89248320483024832", pMsgData->datalen);
		enqueue_serial2wifi_msgbuff(pMsgData);
		OSAL_TaskDelay(1000);
	}
