...
static INT32 init_buffers(int qid, int size, int count)
{
    ...
    */
	...
	printf("qid=%d, buff=%p, head=%p\n", qid, bQueue[qid].pBuffInfo, bQueue[qid].pBuffHead);
...
PSERIAL2WIFI_MSG_DATA _alloc_serial2wifimsgbuff(char*file, int line)
{

	PSERIAL2WIFI_MSG_DATA pMsgData;

	pMsgData = (PSERIAL2WIFI_MSG_DATA)alloc_buffer(INFO_BUFF_ID, file, line);
	if(NULL == pMsgData)
		return NULL;
	
	memset((void*)pMsgData, 0, sizeof(PSERIAL2WIFI_MSG_DATA));
	
	return pMsgData;
}

INT32 _free_serial2wifimsgbuff(void *ptr, char*file, int line)
{
...
/*serial2wifi msgdata 入队列*/
INT32 enqueue_serial2wifi_msgbuff(PSERIAL2WIFI_MSG_DATA pMsgData)
{
	pthread_mutex_lock(&bQueue[INFO_BUFF_ID].qlock);	
	InsertTailList(&bQueue[INFO_BUFF_ID].Qlist, &pMsgData->node);
...
/*serial2wifi msgdata 入队列*/
PSERIAL2WIFI_MSG_DATA dequeue_serial2wifi_msgbuff(void)
{
    PLIST_ENTRY pListEntry;
	...
	return (PSERIAL2WIFI_MSG_DATA)pListEntry;
}
