...
/*更新maxfd*/
static int update_maxfd(fd_set fds, int maxfd) 
{	
	int i;
	int new_maxfd = 0;
	for (i = 0; i <= maxfd; i++)
...
UINT CPCWIFIDlg::OnStartTcpServer(LPVOID lpParam)
{
	...
	FD_ZERO(&readfds_bak);
	/*将服务器socket加入到集合中*/
	FD_SET(slisten, &readfds_bak);

	SOCKET maxfd = slisten;
	...
	while(g_serial2wifi.flag)   
	{
		...
		{  
			...
			{	
				...
				{
					/*客户端socket中有数据请求时*/
					int ret = recv(i, recvData, sizeof(recvData), 0);
					if(ret == 0)
					{
						closesocket(i);
					...
					if(ret > 0)
					{
						...
						send(i, sendData, strlen(sendData), 0);
					}
				}	
			}	
		}	
