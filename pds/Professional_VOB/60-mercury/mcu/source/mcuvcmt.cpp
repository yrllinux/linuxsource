...
//     {
	...
	//需要给自己发哑音包的终端（如sony 终端）给所挂mp板发MCU_MP_DELSENDSELFMUTEPACK_CMD,让其停止给自己发哑音包
	u8 byManuId = m_ptMtTable->GetManuId(tMt.GetMtId());
	if (  g_cMcuVcApp.IsNeedSendSelfMutePack(byManuId,m_ptMtTable->GetProductId(tMt.GetMtId())) )
	{
		//getmpid
		u8 byMpId = m_ptMtTable->GetMpId(tMt.GetMtId());
		g_cMpManager.DelSendSelfMutePack(byMpId,tMt);	
	}
...
//            RefreshRcvGrp(tMt.GetMtId());
		...
		if( tLogicChnnl.GetMediaType() == MODE_AUDIO)
		{
			...
			//给自己发哑音包的终端（如sony终端）开前向音频通道的时候给mp发MCU_MP_ADDSENDSELFMUTEPACK_CMD
			u8 byManuId = m_ptMtTable->GetManuId(tMt.GetMtId());
			if ( g_cMcuVcApp.IsNeedSendSelfMutePack(byManuId,m_ptMtTable->GetProductId(tMt.GetMtId())) )
			{
				TSendMutePack tSendMutePack;
				tSendMutePack.SetConfIdx(m_byConfIdx);
				tSendMutePack.SetMtId(tMt.GetMtId());
				tSendMutePack.SetDstIp(tLogicChnnl.GetRcvMediaChannel().GetIpAddr());
				tSendMutePack.SetDstPort(tLogicChnnl.GetRcvMediaChannel().GetPort());
				tSendMutePack.SetPayloadType(tLogicChnnl.GetChannelType());
				TAudioTypeDesc tAudioTypeDes = m_tConfEx.GetMainAudioTypeDesc();
				tSendMutePack.SetAudioTrackNum(tAudioTypeDes.GetAudioTrackNum());
				u8 byMpId = m_ptMtTable->GetMpId(tMt.GetMtId());
				g_cMpManager.AddSendSelfMutePack(byMpId,tSendMutePack);			
			}
						...
						if (SUCCESS_AGENT == wRet)
						{
							...
							for (u8 byTvChlLp = 0; byTvChlLp < MAXNUM_TVWALL_CHNNL_INSMOUDLE; byTvChlLp++)
							{
								if( m_tConfEqpModule.IsMtInTvWallChannel( byTvWallId, byTvChlLp, tMt, byMemberType) &&
									...
									byMemberType == TW_MEMBERTYPE_NULL )
								{
	...
	case STATE_ONGOING:
		{
			...
			if (tMt.GetMtType() == MT_TYPE_VRSREC)
			{
				...
				//刷新放像终端信息，更新mttype
				m_tPlayEqp = m_ptMtTable->GetMt(m_tPlayEqp.GetMtId());
	...
	if( FALSE == bDecodeAud && TRUE == bOpenFlag )
	{
		...
		{			
				...
				//if( (tAudSrcMt.GetMtId() == byDstMtId) && (m_tConf.GetSpeaker().IsLocal() && IsSupportMultiSpy() || !IsSupportMultiSpy()))
			{
				...
				if (byDstMtNum > 0)
				{
					StopSwitchToSubMt(byDstMtNum, atDstMt, MODE_AUDIO, FALSE, FALSE);
				}
