        ...
        if (g_cMcuVcApp.IsMpConnected(wMpId+1) || (MAXNUM_DRI == wMpId))
        {
            ...
            if (byMode == MODE_VIDEO || byMode == MODE_BOTH)
            {
                ...
                bResult1 = StartSwitch(tSrc, tSrc.GetConfIdx(), dwSrcIp, 0, dwRcvIp, wRcvPort,
                                       dwMultiCastAddr, dwMultiCastPort, dwRcvIp, /*dwRcvIp*/dwSndBindIp);
                if (!bResult1)
                {
            ...
            if (byMode == MODE_AUDIO || byMode == MODE_BOTH)
            {
                ...
                bResult2 = StartSwitch(tSrc, tSrc.GetConfIdx(), dwSrcIp, 0, dwRcvIp, (wRcvPort+2),
                                       dwMultiCastAddr, (dwMultiCastPort+2), dwRcvIp, /*dwRcvIp*/dwSndBindIp);
                if (!bResult2)
                {
            ...
            if (byMode == MODE_SECVIDEO)
            {
                bResult1 = StartSwitch(tSrc, tSrc.GetConfIdx(), dwSrcIp, 0, dwRcvIp, (wRcvPort+4),
                                       dwMultiCastAddr, (dwMultiCastPort+4), dwRcvIp, /*dwRcvIp*/dwSndBindIp);
                if (!bResult1)
                {
...
/*=============================================================================
函 数 名： AddSendSelfMutePack
功    能： 增加一条给自己发哑音包的条目
算法实现： 
全局变量： 
参    数：	u8 byMpId，要往哪块转发板发
const TSendMutePack & tSendMutePack, 要给自己发哑音包的相关信息

返 回 值： void 
-----------------------------------------------------------------------------
修改记录：
日  期		版本		修改人		走读人    修改内容
2012/10/22  4.7			zhouyiliang                  创建
=============================================================================*/
void CMpManager::AddSendSelfMutePack(const u8& byMpId,const TSendMutePack& tSendMutePack)
{
	CServMsg cMsg;
	cMsg.SetMsgBody((u8*)&tSendMutePack,sizeof(TSendMutePack));
	cMsg.SetConfIdx( tSendMutePack.GetConfIdx() );
	g_cMpSsnApp.SendMsgToMpSsn(byMpId, MCU_MP_ADDSENDSELFMUTEPACK_CMD, cMsg.GetServMsg(), cMsg.GetServMsgLen());
}

/*=============================================================================
函 数 名： DelSendSelfMutePack
功    能： 删除一条给自己发哑音包的条目
算法实现： 
全局变量： 
参    数：  const u8 & byMpId ,要给哪块转发板发
const TMt & tMt 要删除的终端
  ...
  2012/10/22  4.7			zhouyiliang                  创建
=============================================================================*/
void CMpManager::DelSendSelfMutePack(const u8 & byMpId,const TMt & tMt)
{
	CServMsg cMsg;
	cMsg.SetMsgBody((u8*)&tMt,sizeof(TMt));
	cMsg.SetConfIdx(tMt.GetConfIdx());
	g_cMpSsnApp.SendMsgToMpSsn(byMpId, MCU_MP_DELSENDSELFMUTEPACK_CMD, cMsg.GetServMsg(), cMsg.GetServMsgLen());
}
...
BOOL32 CMpManager::StartSwitchToBrd( const TMt &tSrc, u8 bySrcChnnl, BOOL32 bForce, u16 wSpyStartPort,u8 bySpecMpId)
{
    ...
    else
	{
		...
		LogPrint(LOG_LVL_KEYSTATUS, MID_MCU_MPMGR, "Send MCU_MP_ADDBRDSRCSWITCH_REQ from RecvMp %s:%d to all Mp.\n",
    ...
    for (u8 byMpLoop = 1; byMpLoop <= MAXNUM_DRI; byMpLoop ++)
    {
		...
		for( u8 bySrcChnl = 0; bySrcChnl < bySwitchGrpNum; bySrcChnl++ )
		{
			...
			for ( u8 byMtLoop = 0; byMtLoop < byDstMtNum; byMtLoop ++ )
			{
				...
				LogPrint( LOG_LVL_KEYSTATUS, MID_MCU_MPMGR, "Add Mt.%d(%s:%d) in Msg to Mp.%d \n",
					pDstMt[byMtLoop].GetMtId(),
					StrOfIP(tSwitchChannel.GetDstIP()), tSwitchChannel.GetDstPort(), byMpLoop);
			}
		}
                             ...
                             u8 byIsSwitchRtcp, BOOL32 bStopBeforeStart,u8 byDstMtId)
{
    ...
    tSwitchChannel.SetRcvIP( dwRcvIp );
	tSwitchChannel.SetRcvPort( wRcvPort );
	LogPrint(LOG_LVL_DETAIL, MID_MCU_MPMGR, "[StartSwitch] dwRcvBindIP: %x  dwSndBindIp:%x\n", dwRcvBindIP, dwSndBindIP);
...
----------------------------------------------------------------------
	...
	04/05/12    3.0         胡昌威        创建
====================================================================*/
BOOL32 CMpManager::AddRecvOnlySwitch( u8 byConfIdx, u32 dwRcvIp, u16 wRcvPort,u32 dwRtcpBackIp,u16 wRtcpBackPort, BOOL32 bReverseG7221c)
{
	//根据IP地址查找交换MP编号
	u8 byMpId = g_cMcuVcApp.FindMp( dwRcvIp );
	...
	u8 byReverseG7221c = bReverseG7221c;
	...
	cServMsg.SetDstDriId( byMpId );
	cServMsg.SetMsgBody((u8   *)&tSwitchChannel,sizeof(tSwitchChannel));
	cServMsg.CatMsgBody(&byReverseG7221c, sizeof(byReverseG7221c));
	StaticLog("Send MCU_MP_ADDRECVONLYSWITCH_REQ to mp!! byReverseG7221c.%d\n", byReverseG7221c);
	...
	if( byMode == MODE_AUDIO || byMode == MODE_BOTH )
	{
			...
			MT_MANU_KDCMCU != ptConfMtTable->GetManuId(tMt.GetMtId()) )
		{
			...
			if (ptConfMtTable->GetMtLogicChnnl(tMt.GetMtId(),LOGCHL_AUDIO,&tLogicChannel,FALSE))
			{
				...
				// 针对G7221C音频格式判码流内容是否需要奇偶对换
				if (MEDIA_TYPE_G7221C == tLogicChannel.GetChannelType())
				{
					u8 byManuId = ptConfMtTable->GetManuId(tMt.GetMtId());
					bReverseG7221c = g_cMcuVcApp.IsReverseG7221c(byManuId,ptConfMtTable->GetProductId(tMt.GetMtId()));
				}
			}
		...
		bResult2 = AddRecvOnlySwitch( tMt.GetConfIdx(), dwRcvIp, wRcvPort+2 ,dwRtcpBackIp,wRtcpBackPort, bReverseG7221c);
		if(!bResult2)
		{
...
void CMpManager::ShowMtAdp()
{
	...
	for( byLoop = 0; byLoop < MAXNUM_DRI; byLoop++)
	{
		if( g_cMcuVcApp.m_atMtAdpData[byLoop].m_bConnected )
		{
            ...
            StaticLog( "\nMtAdp%d: %s (MaxMT/SMcu/HDMt/AUDMt ||OnlineMt/SMcu/All: %d/%d/%d/%d ||%d/%d/%d)", byLoop+1, 
