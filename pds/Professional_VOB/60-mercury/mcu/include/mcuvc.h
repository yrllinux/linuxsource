...
//    void ChangeConfRestoreMixParam();
	...
	BOOL32  IsSrcVidCapLowerDst(TVideoStreamCap tSrcVidCap, TVideoStreamCap tDstVidCap);//比较两个视频能力大小，其中HP>BP
...
#if defined(_8KE_) || defined(_8KH_)	|| defined(_8KI_)// 8000系列内存不吃紧，还是64个
	#define   MAXNUM_VCINSTANCE          MAXNUM_MCU_CONF
#elif defined(_LINUX_)
	#ifdef _MINIMCU_
		#define     MAXNUM_VCINSTANCE            16 //8000b 16个vcinstance
	#else
	#ifdef _LINUX12_
		#define     MAXNUM_VCINSTANCE        MAXNUM_MCU_CONF //MPC2 内存足够，64个instance
	#else
		#define     MAXNUM_VCINSTANCE            16  //mpc 内存不足，缩小为16个
	#endif 
	#endif
#else   // WIN32
	#define     MAXNUM_VCINSTANCE                MAXNUM_MCU_CONF //win32 
#endif


typedef zTemplate< CMcuVcInst, MAXNUM_VCINSTANCE, CMcuVcData, sizeof( CConfId ) > CMcuVcApp;	//Daemon实例用于处理非会议命令
