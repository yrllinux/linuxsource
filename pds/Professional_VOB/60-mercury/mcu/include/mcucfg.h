...
struct TAgentMcuGenCfg
{
	...
	TMultiManuNetCfg	m_tMultiManuNetCfg;	//多运营商网络配置
};
...
public:
	...
	void  ProcDscCfgWaitingChangeOverTimerOut( void ); 
...
//    u8      BrdTypeIn2Out(u8 byType);
    ...
    BOOL32  BrdCfgIn2Out(TBrdCfgInfo *ptIn, TBoardInfo *ptOut, u8 byNum = 1,u8 *pbyRealNum = NULL );
    ...
    BOOL32  GeneralCfgOut2In(TAgentMcuGenCfg &tIn, TMcuNewGeneralCfg &tOut);
    
	BOOL32  GeneralCfgIn2Out(TMcuNewGeneralCfg &tIn, TAgentMcuGenCfg &tOut);
	...
	void    SendExtMcuStatus2Mcsssn( u8 byMcsId);
...
private:
	...
	//void    ReConstructTMcuEqpCfgForMcs( TMcu8KECfg & tMcuEqpCfg, TMcuEqpCfg &tEqpCfg );
	void	ReConstructTMcuNetCfgForMcs(TMcu8KECfg &tMcuEqpCfg, TMcuNetCfg &tMcuNetCfg);
	//void	SetMultiNetCfgToSys(TMultiNetCfgInfo &tMultiNetCfgInfo);

	void	JudgeHduSchemeInConf( const u8* pbySchemeIdx, u8* const pbySchemeInConf, const u8 bySchemeNum );
	BOOL32	IsHduSchmeInConf( const u8 bySchemeIdx, const TTemplateInfo& tTemplateInfo ) const;
	void    WriteMixerCfgToFile( u8 byMixNum, TEqpMixerInfo* ptMixerTable );
