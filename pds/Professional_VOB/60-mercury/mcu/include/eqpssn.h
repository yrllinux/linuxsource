...
public:
	...
	void Proc8KEEqpMcuRegMsg( const CMessage *pcMsg);
	void Proc8KiMixerMcuRegMsg( const CMessage *pcMsg);
	void DaemonProcEqpMcuRegMsg(const CMessage *pcMsg);
	void DaemonProc8KMixerMcuRegMsg(const CMessage *pcMsg);
	...
	BOOL32 IsEqpReged( const u8 &byEqpType, const u32 &dwEqpIp );
    u8   GetIdleInst( const u8 &byEqpType,u8 byIsAudBas = 0 );
