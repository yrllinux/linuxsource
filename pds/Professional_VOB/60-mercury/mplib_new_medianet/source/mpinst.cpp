...
void CMpInst::ProcPowerOn()
{
    ...
    {
        MpLog( LOG_CRIT, "KdvSocketStartup failed, error: %d\n", wRet );
...
BOOL IsAddMuti(u32 dwIpAddr)
{
	u8 byIpHead = (u8)(dwIpAddr >> 24);
	return byIpHead >= 224 && byIpHead <= 239;
}
...
void CMpInst::ProcAddSwitchReq(CMessage * const pcMsg)
{
    ...
    u8  bySwitchChannelMode = *(u8 *)(cRevMsg.GetMsgBody() + sizeof(TSwitchChannel));
    u8  byUniformPayload    = *(u8 *)(cRevMsg.GetMsgBody() + sizeof(TSwitchChannel) + sizeof(u8));
    u8  byIsSwitchRtcp      = *(u8 *)(cRevMsg.GetMsgBody() + sizeof(TSwitchChannel) + sizeof(u8) + sizeof(u8));
	...
	BOOL32 bIsDstMuti = IsAddMuti(dwDstIp);
	...
	{
		//FIXME: 1、临时构造本地RcvPort, 转给本地net收发对
		dwResult1 = ::dsAdd( g_cMpApp.m_nInitUdpHandle, 
								htonl(dwRevIp), wRevPort, htonl(dwRevBindIp), 
								htonl(dwRevIp), wRevPort % 10000 + RCV_PORT_OFFSET, htonl(dwSndBindIp) );

		u8 byNetSW = g_cNetSWGrp.GetIdleNetSW();
		if (byNetSW == 0xff)
		{
			MpLog( LOG_WARN, "[ProcAddSwitchReq] switch: %s(%d)-->%s(%d) get NetSw failed!\n", 
                          achRevIp, wRevPort, strofip(dwDstIp), wDstPort ); 
			return;
		}
		g_cNetSWGrp.InitNetSW(byNetSW, tSwitchChannel, MEDIA_TYPE_H264);
	}
...
void CMpInst::ProcRemoveSwitchReq(CMessage * const pcMsg)
{
    ...
    TSwitchChannel tSwitchChannel = *(TSwitchChannel*)cRevMsg.GetMsgBody();
    u8  byIsSwitchRtcp = *(u8 *)(cRevMsg.GetMsgBody() + sizeof(TSwitchChannel));
    u32 dwDstIp  = tSwitchChannel.GetDstIP();
    u16 wDstPort = tSwitchChannel.GetDstPort();
	...
	{
		//FIXME: 拆本交换需要知道源终端的ID
		/*
		dwResult1 = ::dsRemove(g_cMpApp.m_nInitUdpHandle,
							   htonl(g_cMpApp.m_dwIpAddr), wDstPort + RCV_PORT_OFFSET);
		*/
		g_cNetSWGrp.QuitNetSW(tSwitchChannel, MEDIA_TYPE_H264);
	}
...
void CMpInst::ProcAddMulToOneSwitchReq(CMessage * const pcMsg)
{
	...
	u16 wDstPort    = tSwitchChannel.GetDstPort();
	u32 dwSndBindIP = tSwitchChannel.GetSndBindIP();
	u32 dwMapIpAddr = *(u32*)(cRevMsg.GetMsgBody()+sizeof(TSwitchChannel));
	u16 wMapPort    = *(u16*)(cRevMsg.GetMsgBody()+sizeof(TSwitchChannel)+sizeof(u32));
...
//8000H/G/I不需要修改RecvIP[5/25/2012 chendaiwei]
    ...
    u8  bySwitchChannelMode = SWITCHCHANNEL_UNIFORMMODE_VALID;
...
void CMpInst::ProcAddBroadSwitchDstReq(CMessage * const pcMsg)
{
    ...
    if (wMsgBodyLen % (sizeof(TSwitchChannel) + sizeof(u8) * 3) != 0)
    {
    ...
    u8 byDstMtNum = wMsgBodyLen / (sizeof(TSwitchChannel) + sizeof(u8) * 3);
    ...
    {
        TSwitchChannel tSwitchChannel = *(TSwitchChannel*)(cServMsg.GetMsgBody() + wOffSet);
        u8  bySwitchChannelMode = *(u8*)(cServMsg.GetMsgBody() + sizeof(TSwitchChannel) + wOffSet);
        u8  byUniformPayload    = *(u8*)(cServMsg.GetMsgBody() + sizeof(TSwitchChannel) + 1 + wOffSet);
        u8  byIsSwitchRtcp      = *(u8*)(cServMsg.GetMsgBody() + sizeof(TSwitchChannel) + 2 + wOffSet);
     
        wOffSet += sizeof(TSwitchChannel) + 3;
        ...
        if ( g_cMpApp.SearchSwitchTable(byConfIdx, dwDstIp, wDstPort, tTmpChan) )
        {
            ...
            if ( g_cMpApp.m_dwIpAddr == tTmpChan.GetRcvIP() &&
...
=============================================================================*/
void CMpInst::ProcNetStat(CMessage* const pcMsg)
{
...
=============================================================================*/
void CMpInst::ProcFluxOverRun(CMessage* const pcMsg)
{
	CServMsg cServMsg;
    cServMsg.SetSrcDriId( g_cMpApp.m_byMpId );
