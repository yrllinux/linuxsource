...
void CNetTrfcStat::DumpAllStat()
{
    ...
    s32 lnRecv, lnSend;
    for ( s32 i = 0; i < NETFRFCSTAT_BUFFER/NETFRFCSTAT_TIMESPAN; i++ )
    {
        lnRecv = (s32)(((m_alnNetRecvBytes[i] - m_alnNetRecvBytes[i+1]) / NETFRFCSTAT_TIMESPAN) >> 7);  // Byte -->大约Kbps
...
void  CNetTrfcStat::GetAvgNetBurden(u32 nTimeSpan, s32 &nSend, s32 &nRecv)
{
    ...
    nRecv = (s32)(lnRecv / (nTimeSpan/NETFRFCSTAT_TIMESPAN));
    nSend = (s32)(lnSend / (nTimeSpan/NETFRFCSTAT_TIMESPAN));
    return ;
}
