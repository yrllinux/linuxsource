        ...
        else
        {
            ...
            else
            {
                    ...
                    m_pptSwitchChannel[byConfIdx][wTableIdx].GetDstPort() == wDstPort )
                {
                    ...
                    u16 wRcvPort = m_pptSwitchChannel[byConfIdx][wTableIdx].GetRcvPort();
                    u32 dwRcvIp  = m_pptSwitchChannel[byConfIdx][wTableIdx].GetRcvIP();
...
BOOL32 CMpData::IsTInUseStill( u8 byConfIdx )
{
    ...
    for( u8 byChnIdx = 0; byChnIdx < MAX_SWITCH_CHANNEL; byChnIdx ++ )
    {
        if ( 0 != m_pptSwitchChannel[byConfIdx][byChnIdx].GetDstIP() &&
             0 != m_pptSwitchChannel[byConfIdx][byChnIdx].GetDstPort() )
        {
            if ( g_cMpApp.m_dwIpAddr == m_pptSwitchChannel[byConfIdx][byChnIdx].GetRcvIP() &&
                 CONFBRD_STARTPORT+byConfIdx*PORTSPAN == m_pptSwitchChannel[byConfIdx][byChnIdx].GetRcvPort() )
            {
                byMtDataFromT ++;

                MpLog( LOG_INFO, "[IsTInUseStill] Conf.%d: Rcv.0x%x@%d --> Dst.0x%x@%d !\n",
                                  byConfIdx,
                                  m_pptSwitchChannel[byConfIdx][byChnIdx].GetRcvIP(),
                                  m_pptSwitchChannel[byConfIdx][byChnIdx].GetRcvPort(),
                                  m_pptSwitchChannel[byConfIdx][byChnIdx].GetDstIP(),
                                  m_pptSwitchChannel[byConfIdx][byChnIdx].GetDstPort());
            }
        }
    }
