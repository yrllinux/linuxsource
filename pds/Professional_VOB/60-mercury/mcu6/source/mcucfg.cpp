...
void  CMcuCfgInst::ProcMcuEqpCfg(const CMessage * pcMsg)
{
	...
	else
	{
...
#if defined(_8KI_)
		if (sizeof(TNewNetCfg) != cServMsg.GetMsgBodyLen())
		{
		...
		memcpy(&tNewNetCfg, pbyMsg, sizeof(tNewNetCfg));
		...
		//设置网口信息
		g_cMcuAgent.SetNewNetCfgInfo(tNewNetCfg);
		
		TEthCfg tEthCfg;
		TNetCfg tNetCfg;
		
		//第0个网口的第0个IP为MCU IP
		tNewNetCfg.GetEthCfg(0, tEthCfg);			//取第0个网口信息
		tEthCfg.GetNetCfg(0, tNetCfg);				//取第0个IP信息
		u32 dwMcuIp = tNetCfg.GetIpAddr();			//取McuIp
		
		u32 dwSipIp = tNewNetCfg.GetSipIpAddr();	//取SipIp
		
		//内置GK IP、Sip Ip与MCU IP出现相同的情况下，为避免IP冲突，将mcu内置接入的RASport、Q931 port
		//和Sip port置为1919、1920和1720
		g_cMcuAgent.AdjustNewMcuRasPort(dwMcuIp, tGkPxyInfo.GetGkIpAddr(), dwSipIp);
#else
		////////////////////////////////////////////////////////////
		...
		TMcuNetCfg *ptMcuNetCfg = (TMcuNetCfg *)cServMsg.GetMsgBody();
		...
		g_cMcuAgent.AdjustMcuRasPort(ptMcuNetCfg->m_atMcuEqpCfg[1].GetMcuIpAddr(),tGkPxyInfo.GetGkIpAddr());
		...
		for(u8 byLp = 0; byLp < MAXNUM_ETH_INTERFACE; byLp ++)
		{
			...
			TNetAdaptInfo *ptNetAdatpInfo = tMcu8KECfg.m_tLinkNetAdap.GetValue(byLp);
...
void  CMcuCfgInst::ProcMcuGeneralCfg(const CMessage * pcMsg)
{
	...
	else
	{
		...
		if (wMsgLen > sizeof(TMcuGeneralCfg) + sizeof(u8))
		{
			...
			if (wMsgLen > sizeof(TMcuGeneralCfg) + sizeof(u8)+sizeof(tNewGenCfg.m_achRRQPassword)+sizeof(tNewGenCfg.m_byIsGkUseRRQPwd))
			{
...
// 		}
		...
		//多运营商网络配置
		wRet = g_cMcuAgent.SetMultiManuNetCfg(tAgentGenCfg.m_tMultiManuNetCfg);
...
#if defined(_8KI_) 
	TNewNetCfg tEqpCfg;
	g_cMcuAgent.GetNewNetCfgInfo(tEqpCfg);
#else
	TMcu8KECfg tMcu8KECfg;
	g_cMcuAgent.GetMcuEqpCfg(&tMcu8KECfg);
...
BOOL32 CMcuCfgInst::NotifyMcsGeneralCfg( void )
{
	...
	cMsg.CatMsgBody((u8*)&tNewNetCfg, sizeof(tNewNetCfg));
#endif
...
void CMcuCfgInst::NotifyMcsBoardStatus(const CMessage * pcMsg)
{
    ...
    else if (AGT_SVC_UPDATEBRDVERSION_NOTIF == pcMsg->event)
    {
		...
		{
			//如果是主板上报所有单板升级通知给MCS
			//备办只上报MPC或MPC2升级通知，因为主板不会上报备办升级通告
