...
//#ifndef _8KE_
    ...
    */
	...
	{
		#ifdef _8KH_
			//根据License描述，记录是800L还是8000H类型 [6/18/2012 chendaiwei]
			if( 0 == strcmp( "800L", achGetValue ) )
			{
				printf( "read license Mcu type is %s!\n", achGetValue);
				g_cMcuAgent.SetMcuType800L();
				OspSetPrompt("mcu_800l");
			}
			else if ( 0 == strcmp("8000H-M",achGetValue))
			{
				printf( "read license Mcu type is %s!\n", achGetValue);
		...
		case MCU_TYPE_KDV8000I:
			...
			break;
		case MCU_TYPE_KDV800L:
			//前面已经比较过，800L类型不需要再比较 [6/25/2012 chendaiwei]
			break;
		case MCU_TYPE_KDV8000H_M:
			break;
		default:
