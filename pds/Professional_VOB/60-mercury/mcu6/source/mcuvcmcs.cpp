...
// 				}
			...
			if(!m_cCfgInfoRileExtraDataMgr.ReadExtraDataFromConfInfoFile(m_byConfIdx,tConfAttrbEx.IsDefaultConf(), bIsConfFileGBK))
			{
...
BOOL32 CMcuVcInst::IsConfFileGBK()
{
	...
	LogPrint(LOG_LVL_KEYSTATUS, MID_MCU_CONF, "[IsConfFileGBK]conf.%d fileidx.%d name.%s pack len.%d\n",
		m_byConfIdx, byFileIndex, tConfStore.m_tConfInfo.GetConfName(), wPackConfDataLen);
...
void CMcuVcInst::ProcStopConfPoll(BOOL32 bNeedAdjustVmp)
{
		...
		( MODE_VIDEO_CHAIRMAN == byPollMeida || MODE_BOTH_CHAIRMAN == byPollMeida ))		
    {
        ...
        case MODE_BOTH:
            ...
            //拆本地发言
            tMt = m_tConf.GetSpeaker();
			if (!IsMultiCastMt(GetLocalSpeaker().GetMtId()) || IsMultiCastMt(tLocalPollMt.GetMtId()) || GetCurSatMtNum() > 0)
			{
				ChangeVidBrdSrc(&tMt);
			}
        ...
        case MODE_VIDEO_BOTH:
            {
				...
				//恢复本地视频广播
				if(HasJoinedSpeaker() && (!IsMultiCastMt(GetLocalSpeaker().GetMtId()) || IsMultiCastMt(tLocalPollMt.GetMtId()) || GetCurSatMtNum() > 0))
				{
					tMt = m_tConf.GetSpeaker();
					ChangeVidBrdSrc(&tMt);
				}
...
//         }
		...
		{
			// [2013/03/11 chenbing] HDU多画面目前不支持轮询跟随,子通道置0
			ChangeHduSwitch(&tSrcMt, byHduId, byDstChnnlIdx, 0,	TW_MEMBERTYPE_POLL, TW_STATE_START, 
				m_tConf.m_tStatus.GetPollMedia() );
		}
		else
		{
			ChangeHduSwitch(&tSrcMt, byHduId, byDstChnnlIdx, byHduSubChnId,\
				tHduSwitchInfo.GetMemberType(), TW_STATE_START, MODE_BOTH, FALSE, TRUE, TRUE, byHduMode);
		}
        break;
    }
    default:
