...
{	
	...
	if ( lpszTable == NULL )
	{
		OspPrintf(TRUE, FALSE, "[ReadMtIpMapTab]alloc memory failed\n");
		return FALSE;
	}
...
int get_linklocal_ipv6addr(char *address, s16 &swScopeId, char *iface)
{
	...
	char * pbyV6Ip = "fe80::be30:5bff:feca:ae04";
...
#define IPV6_ADDR_LINKLOCAL 0x0000U
	while (fgets(str, sizeof(str), fp)) {
		...
		if (IPV6_ADDR_LINKLOCAL != (unsigned int)strtoul(scope, NULL, 16))
			// 测试用,所以注释；正常要放开 [pengguofeng 6/1/2012]
// 			continue;
// 		memset(address, 0x00, sizeof(address)); // 由外面保证；或者加一个参数表示长度 [pengguofeng 6/14/2012]
		p = addr;
		q = address;
		count = 0;
		while (*p != ' ') {
		...
		if( m_atMtExt[byMtId - 1].GetMtId() == byMtId )
		{
            ...
            if ( mtAliasTypeTransportAddress == ptAlias->m_AliasType )
            {
                if ( ptAlias->m_tTransportAddr.GetIpAddr() == 
					m_atMtData[byMtId-1].m_atMtAlias[ptAlias->m_AliasType-1].GetMtAlias().m_tTransportAddr.GetIpAddr() )
                {
					if ( m_atVrsRecChlStatus[byMtId-1].m_byType == TRecChnnlStatus::TYPE_UNUSE)
					{
						// 未使用通道可匹配
						return byMtId;
					}
					else if (m_atVrsRecChlStatus[byMtId-1].m_byType == byType)
					{
						...
						{
							if ( m_atVrsRecChlStatus[byMtId-1].m_tSrc.IsNull() && tSrc.IsNull())
							{
								//匹配会议录像
								return byMtId;
							}
							else if (m_atVrsRecChlStatus[byMtId-1].m_tSrc == tSrc)
							{
								//已使用通道严格匹配录放像模式和源终端
								return byMtId;
							}
...
void McuGetDebugKeyValue(TMcuDebugVal &tMcuDebugVal)
{
	...
	// 读出Ipv6的终端配置 [pengguofeng 6/5/2012]
	s32 nMtEntryNum = 0;
	GetRegKeyInt(achFullName, "IpV6MtTable", "EntryNum", 0, &nMtEntryNum);

	if ( nMtEntryNum > MAXNUM_CONF_MT)
	{
		nMtEntryNum = MAXNUM_CONF_MT;
	}

	if ( nMtEntryNum > 0 )
	{
		for ( u8 byNum = 0; byNum < (u8)nMtEntryNum; byNum++)
		{
			s8 achIP[16] = {0};
			s8 achKey[7] = {0};
			sprintf(achKey, "Entry%d", byNum);

			if ( !GetRegKeyString(achFullName, "IpV6MtTable", achKey, "0.0.0.0", achIP, sizeof(achIP)))
			{
				continue;
			}

			tMcuDebugVal.m_dwIpv6[byNum] = ntohl(INET_ADDR(achIP)); //保存为主机序
		}
	}
...
//#endif
	...
	//交换任务
#if defined(_8KE_) || defined(_8KH_) || defined(_8KI_)
	//一体机只有一块MP板，故直接起新版本,暂时不能和4.7兼容
	// IPV6 [5/12/2012 pengguofeng]
// 	tMp.SetVersion(DEVVER_MPV6);

	//三个ReadDebug接口
	TMtIpMapTab tMtIpMapTab;
	ReadMtIpMapTab(tMtIpMapTab);

	TCriIpMapTab tCriIpMapTab;
	ReadCriIpMapTab(tCriIpMapTab);

	TGKIpMapTab tGkIpMapTab;
	ReadGkIpMapTab(tGkIpMapTab);
	...
	//用新的mpstart传参，mtadp也类似
	if( !mpstart( &tMtIpMapTab, pbyV6Ip, 0, dwMcuIp, wMcuPort, &tMp ) )
	{
...
void StartLocalMtadp( void )
{
	...
	if ( !MtAdpStart(&tMtIpMapTab, &tConnectParam, &tMtadp, &tCriIpMapTab,
		&tGkIpMapTab, pbyV6Ip, swScopeId) )
	//终端适配模块（内嵌方式启动该模块）
	{
...
u8 GetMcuPdtType(void)
{
	...
	if(g_cMcuAgent.Is800LMcu())
	{
		return MCU_TYPE_KDV800L;
	}
	else if(g_cMcuAgent.Is8000HmMcu())
	{
