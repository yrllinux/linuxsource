...
void CMpSsnInst::ProcMpRegisterReq( CMessage * const pcMsg)
{
	...
	case STATE_IDLE:
		...
		{    
			...
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCU_MPMGR, "[ProcMpRegisterReq]MsgLen:%d TMpSize:%d IPV6_len:%d\n",
				cServMsg.GetMsgBodyLen(), sizeof(TMp), IPV6_STR_LEN);
			...
			{
				//校验长度
				if ( cServMsg.GetMsgBodyLen() != sizeof(TMp) + sizeof(u8)* IPV6_STR_LEN
					&& cServMsg.GetMsgBodyLen() != sizeof(TMp) )
				{
