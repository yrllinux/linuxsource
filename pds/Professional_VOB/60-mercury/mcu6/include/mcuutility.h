...
typedef struct TMcuDebugFile
{
	...
	u32		m_dwIpv6[MAXNUM_CONF_MT];	// Ipv6 MT的Ipv4地址 [pengguofeng 6/5/2012]
...
// 卫星变量初始化 [pengguofeng 3/5/2013]
		...
		memset(m_dwIpv6, 0, sizeof(m_dwIpv6));
...
// 卫星会议打印 [pengguofeng 3/5/2013]
		...
		for ( byIndex = 0; byIndex < MAXNUM_CONF_MT; byIndex++)
		{
			if ( m_dwIpv6[byIndex] != 0)
			{
				StaticLog("\tIndex:%d Ipv4[Host]:0x%x\n", byIndex, m_dwIpv6[byIndex]);
			}
		}
#if defined(_8KE_) || defined(_8KH_) || defined(_8KI_)
