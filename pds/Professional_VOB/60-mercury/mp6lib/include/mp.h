...
public:
	...
	// zgc [2007/01/12] 根据指定时间跨度统mcu计流量是否超负载
...
public:
    ...
    BOOL32 SwitchSrcToT( u8 byConfIdx, u8 byPoint, TSwitchChannelV6 &tSrcSwitch, 
                         u8 bySwitchChnMode, u8 byUniformPayload, BOOL32 bSwitchNow = FALSE );

    void ClearInstStatus(void);
    void ShowSwitch(void);
	BOOL32 TransV4MsgtoV6Msg(CMessage* const pcMsg, CServMsg& cServMsg);
	BOOL32 TransV6Chn2V4Chn(TSwitchChannelV6& tSwitchChnnelV6, TSwitchChannel& tSwitchChannel);
protected:
...
public:
    ...
    BOOL32 GetWaitIFrameDataBySrc( s8* pchSrcIp, u16 wSrcPort, TWaitIFrameData *ptWaitData );
    ...
    u16    GetChanNoByDst( u8 byConfIdx, s8* pchDstIp, u16 wDstPort, u8 byGetType, u16 wRcvPort = 0 );

    BOOL32 GetSwitchChan    ( u8 byConfIdx, u16 wIndex, TSwitchChannelV6 *ptSwitchChannel );
    BOOL32 RemoveSwitchTable( u8 byConfIdx, TSwitchChannelV6 *ptSwitchChannel, u8 byType, BOOL32 bStop = FALSE );
    BOOL32 RemoveSwitchTableSrc( u8 byConfIdx, TSwitchChannelV6 *ptSwitchChannel );
    BOOL32 SearchSwitchTable( u8 byConfIdx, s8* pchDstIp, u16 wDstPort, TSwitchChannelV6 &tSwitchChannel );
    BOOL32 IsSrcSwitchExist( u8 byConfIdx, TSwitchChannelV6 *ptSrcSwitch );
//  BOOL32 SearchBridgeSwitch( u8 byConfIdx, u16 wRcvDstPort, TSwitchChannelV6 &tSwitchChannel );

    void   NtfMcuSrcSwitchChg( u8 byConfIdx, TSwitchChannelV6 &tSwitchSrc );
    void   ProceedSrcChange( u8 byConfIdx, u8 byPID, BOOL32 bAnotherPt = TRUE );

    //void   ShowSwitch();
    void   ShowWholeSwitch();
    void   ShowSwitchUniformInfo();
    BOOL32 AddSwitchTable( u8 byConfIdx, TSwitchChannelV6* ptSwitchChannel, u8 byType );
	...
	BOOL32 IsV6MtIp(u32 dwMtIp, u8& byIndex);
	BOOL32 IsV6MtIp(s8* pchMtIp, u8& byIndex);
	s8*    GetV6IpAcd2Tab(u8 byIndex);
	u32    GetV4IpAcd2Tab(u8 byIndex);
...
public:
    ...
    TSwitchChannelV6     (*m_pptSwitchChannel)[MAX_SWITCH_CHANNEL];
	s8 m_achV6Ip[46]; 
	TMtIpMapTab m_tMtIpMapTab;
