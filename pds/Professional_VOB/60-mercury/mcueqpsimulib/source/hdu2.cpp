...
u16	   CKdvVidDec::CreateHD3(TVidDecHD3RD *ptVidDecInit)
{
	return CODEC_NO_ERROR;
}
...
u16    CKdvVidDec::GetDecoderStatus(TKdvDecStatus &tKdvDecStatus,u32 dwChnl)/*获取解码器状态*/
{
	return CODEC_NO_ERROR;
}

u16    CKdvVidDec::GetDecoderStatis(TKdvDecStatis &tKdvDecStatis,u32 dwChnl)/*获取解码器的统计信息*/
{
	return CODEC_NO_ERROR;
}
...
u16    CKdvVidDec::SetData(const TFrameHeader& tFrameInfo,u32 dwChnl)        /*设置视频码流数据*/
{
	return CODEC_NO_ERROR;
}

/*设置视频画中画*/
u16    CKdvVidDec::SetVidPIPParam(enVideoPIPMode enMode, enVideoPIPIndex enIndex[3])
{
	return CODEC_NO_ERROR;
}
...
u16		CKdvVidDec::SetVidPlayPolicy(unsigned long,unsigned long)
{
	return CODEC_NO_ERROR;
}
...
u16	   CKdvAudDec::CreateDecoder(TDecoder *pDecoer,unsigned long)
{
	return CODEC_NO_ERROR;
}
