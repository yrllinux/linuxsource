...
{
	OspPrintf( TRUE, FALSE, "%6u %4u\n", GetInsID(), m_dwMcsNode );
}
...
void CMcsSsnInst::InstanceEntry( CMessage * const pcMsg )
{
	...
	if (NULL == pcMsg)
	{
		OspPrintf( TRUE, FALSE, "CMcsSsnInst: The received msg's pointer in the msg entry is NULL!");
		return;
	}

	if (MCU_MCS_MTSTATUS_NOTIF != pcMsg->event)
	{
		McsLog("[Mcsssn%u]: Event %u(%s) passed!\n", GetInsID(), pcMsg->event, ::OspEventDesc(pcMsg->event));
	}
...
//	case MCU_MCS_UPLOADFILENAME_NACK:
	...
	//转发
	default:
		OspPrintf(TRUE, FALSE, "[InstanceEntry]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
...
void CMcsSsnInst::DaemonInstanceEntry( CMessage * const pcMsg, CApp* pcApp )
{
	...
	default:
		OspPrintf(TRUE, FALSE, "[DaemonInstanceEntry]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
		break;
	}
}
...
void CMcsSsnInst::ProcMcsMcuConnectReq( const CMessage * pcMsg )
{
	...
	McsLog("[Mcsssn]: Message %u(%s) received, InstId=%u!\n",
	...
	case STATE_IDLE:
        {
		    ...
		    if (MCU_MSSTATE_OFFLINE == emLocalMSState)
		    {
                ...
                OspPrintf(TRUE, FALSE, "[Mcsssn][ProcMcsMcuConnectReq]: invalid ms state, NACK.\n");
				...
				&& g_cMcuVcApp.IsVCSNumOverLicense())
			{
                ...
                ::OspDisconnectTcpNode( m_dwMcsNode );
			    m_dwMcsNode = INVALID_NODE;	            
                OspPrintf(TRUE, FALSE, "[Mcsssn][ProcMcsMcuConnectReq]: Over VCS access ability\n");
				return;
			}
...
//                 ::OspDisconnectTcpNode( m_dwMcsNode );
// 			    m_dwMcsNode = INVALID_NODE;	            
//                 OspPrintf(TRUE, FALSE, "[Mcsssn][ProcMcsMcuConnectReq]: MCS VCS login only one\n");
// 				return;
// 			}
            ...
            if (tRegReq.GetMcuMcsVer() != MCUMCS_VER)
            {
                ...
                ::OspDisconnectTcpNode( m_dwMcsNode );
			    m_dwMcsNode = INVALID_NODE;	            
                OspPrintf(TRUE, FALSE, "[Mcsssn][ProcMcsMcuConnectReq]: mcs version(%d) differs from mcu(%d), NACK.\n",
		    ...
		    if (!dwValid)
		    {
			    ...
			    ::OspDisconnectTcpNode( m_dwMcsNode );
			    m_dwMcsNode = INVALID_NODE;
                OspPrintf(TRUE, FALSE, "[Mcsssn][ProcMcsMcuConnectReq]: user name/pwd invalid, NACK.\n");
            ...
            McsLog("[Mcsssn][ProcMcsMcuConnectReq]: User Logged in, Name: %s, Group Id: %d.\n", 
                    tRegReq.GetUser(), 
                    byGrpId );
            ...
            tRsp.SetLocalActive((MCU_MSSTATE_ACTIVE == emLocalMSState));
            ...
            McsLog("[Mcsssn][ProcMcsMcuConnectReq]: Mcs Connect Ack: \n");
            if (g_bPrintMcsMsg)
            {                
                tRsp.Print();
            }
	...
	default:
		OspPrintf(TRUE, FALSE, "[ProcMcsMcuConnectReq]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
		break;
	}
}
...
void CMcsSsnInst::ProcMcsMcuGetMsStatusReq( const CMessage * pcMsg )
{
    ...
    {
        OspPrintf(TRUE, FALSE, "[ProcMcsMcuGetMsStatusReq]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
            pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
    ...
    if( MCU_MSSTATE_ACTIVE == g_cMSSsnApp.GetCurMSState() )
    {
        ...
        McsLog("[ProcMcsMcuGetMsStatusReq] IsMsSwitchOK :%d.\n", tMsStatus.IsMsSwitchOK());
    }
...
void CMcsSsnInst::ProcMcsInquiryMcuTime( const CMessage * pcMsg )
{
	...
	McsLog("[Mcsssn]: Message %u(%s) received, Current State %d!\n",
	...
	case STATE_NORMAL ://连接建立
		...
		break;
	default :
		OspPrintf(TRUE, FALSE, "[ProcMcsInquiryMcuTime]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
		break;
	}
...
void CMcsSsnInst::ProcMcsMcuChgSysTime( const CMessage * pcMsg)
{
    ...
    {
        OspPrintf(TRUE, FALSE, "[ProcMcsMcuChgSysTime] msg body is invalid! len: %d\n", cMsg.GetMsgBodyLen());
        return;
    }
    ...
    if (g_cMcuVcApp.GetMcuExpireDate().GetYear() != 0)
    {
        ...
        McsLog("can't syn time for mcu expire date.\n");
        return;
    }
    ...
    {	
		...
		cMsg.SetSrcSsnId( ( u8 )GetInsID() );
		SendMsgToMcs(pcMsg->event + 2, cMsg.GetServMsg(), cMsg.GetServMsgLen());
		OspPrintf(TRUE, FALSE, "[ProcMcsMcuChgSysTime] change system time failed, ret: %d\n", wRet);        
    }
    else
    {		
		TKdvTime tCurMcuTime;
...
void CMcsSsnInst::ProcMcsDisconnect( const CMessage * pcMsg )
{
    ...
    McsLog("[Mcsssn%u]: Message %u(%s) received!\n", GetInsID(), pcMsg->event, ::OspEventDesc(pcMsg->event));
...
//    McsLog("[SendMsgToMcs] confid :");
//    cMsg.GetConfId().Print();
    McsLog("[SendMsgToMcs] Event %u(%s) To Console %u\n", wEvent, OspEventDesc(wEvent), GetInsID() );
	...
	{
		OspPrintf( TRUE, FALSE, "CMcsSsnInst: ABNORMAL McSsn: Node=%u(IP: 0x%x), CurState=%u, InstId=%d\n", 
...
void CMcsSsnInst::ProcMcsMcuMsg(const CMessage * pcMsg)
{
    ...
    McsLog("Command message %u(%s) from Console.%u received!\n", 
              pcMsg->event, ::OspEventDesc(pcMsg->event), GetInsID() );
...
void CMcsSsnInst::ProcMcuMcsMsg( const CMessage * pcMsg, BOOL32 bCheckGrp )
{
	...
	McsLog("[Mcsssn%u]: Command reply Message %u(%s) to Console received!\n",
			GetInsID(), pcMsg->event, ::OspEventDesc(pcMsg->event) );
	...
	case STATE_NORMAL:
        ...
        {
            McsLog( "[ProcMcuMcsMsg]: Message %u(%s) In InsID.%u is Dropped for CConf Group Id(%d) <--> Ssn Group Id(%d)\n",
                    pcMsg->event, ::OspEventDesc( pcMsg->event ), GetInsID(), 
		...
		break;
	default:
		OspPrintf(TRUE, FALSE, "[ProcMcuMcsMsg]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
		break;
	}
}
...
void CMcsSsnInst::ProcMcsMcuDaemonConfMsg( const CMessage * pcMsg )
{
	...
	case STATE_NORMAL:
		...
		break;
	default:
		OspPrintf(TRUE, FALSE, "[ProcMcsMcuDaemonConfMsg]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
		break;
	}
}
...
void CMcsSsnInst::ProcReoOverFlow( const CMessage * pcMsg )
{
	...
	McsLog("[Mcsssn]: Message %u(%s) received, Conference instance full!\n",
	...
	case STATE_NORMAL:
		...
		break;
	default:
		OspPrintf(TRUE, FALSE, "[ProcReoOverFlow]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
		break;
	}
}
...
----------------------------------------------------------------------
    ...
    03/05/29    1.0         
====================================================================*/
void CMcsSsnInst::DaemonProcPowerOn( const CMessage * const pcMsg  )
{
    ...
    {
        OspPrintf(TRUE, FALSE, "MCU: memory allocation for address book failed!\n");
    }
    else
    {
...
BOOL32 CMcsSsn::SendMsgToMcsSsn( u16 wInstId, u16 wEvent, u8 * const pbyMsg, u16 wLen )
{
	...
	{
		OspPrintf(TRUE, FALSE, "CMcsSsn: wrong instance id %u!\n", wInstId);
		return FALSE;
	}
	...
	for (u8 byInstID = 1; byInstID <= MAXNUM_MCU_MC; byInstID++)
	{
        ...
        if ( bySsnGrpId != USRGRPID_INVALID )
        {
            ...
            else
            {
                ...
                McsLog( "[Broadcast2SpecGrpMcsSsn]: Message %u(%s) In InsID.%u is Dropped for Spec Group Id(%d) <--> Ssn Group Id(%d)\n",
...
void CMcsSsnInst::ProcMcuCfgMsg( const CMessage *const pcMsg)
{
	...
	case STATE_NORMAL:
		...
		break;
	default:
		OspPrintf(TRUE, FALSE, "[CMcsSsnInst]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode );
		break;
	}   
...
void CMcsSsnInst::ProcMcuCfgRsp(const CMessage * pcMsg)
{
    ...
    case STATE_NORMAL:
        ...
        break;
    default:
		OspPrintf(TRUE, FALSE, "[ProcMcuCfgRsp]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
        break;
    }   
...
void CMcsSsnInst::ProcMcuCfgFileMsg(const CMessage * pcMsg)
{
    ...
    case STATE_NORMAL:
        ...
        break;
    default:
		OspPrintf(TRUE, FALSE, "[ProcMcuCfgFileMsg]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
			      pcMsg->event, ::OspEventDesc( pcMsg->event ), CurState(), GetInsID(), pcMsg->srcnode);
        break;
    }   
...
void CMcsSsnInst::ProcMcuUnitTestMsg( const CMessage * pcMsg )
{
	...
	case STATE_NORMAL:
		...
		break;
	default:
		OspPrintf(TRUE, FALSE, "[ProcMcuUnitTestMsg]: Wrong inst state in state.%u InsID.%u srcnode.%u!\n", 
...
void CMcsSsnInst::McsMcuRebootCmd( const CMessage * const pcMsg )
{
    ...
    OspPrintf(TRUE, FALSE, "[McsMcuRebootCmd] User Request Reboot MPC IP: 0x%x, cur mpcip:0x%x\n", 
              ntohl(dwMpcIp), ntohl(g_cMcuAgent.GetMpcIp()));
    if(0 != dwMpcIp && dwMpcIp != g_cMcuAgent.GetMpcIp())
    {
        OspPrintf( TRUE, FALSE, "[McsMcuRebootCmd] failed due to mpcip.0x%x, cur mpcip.0x%x\n",
	...
	case MCS_MCU_ADDUSER_REQ:       //会议控制台请求MCU添加用户
        {
            ...
            bRet = g_cMcuVcApp.GetMCSUsrGrpUserCount( byUsrGrpId, byMaxNum, byNum );
            if (byNum >= byMaxNum)
            {
                // 当前组用户满
	...
	case MCS_MCU_GETUSERLIST_REQ:   //会议控制台请求MCU得到用户列表
		{
			...
			if ( !bRet || byUsrNum == 0)
			{
				...
				OspPrintf(TRUE, FALSE, "[ProcUserManagerMsg] No users in current group\n");
			}
			else
			{
				SendMsgToMcs(pcMsg->event+1, cServMsg.GetServMsg(), cServMsg.GetServMsgLen()); // Ack
				...
				while ( cLongMsg.m_uCurr <= cLongMsg.m_uTotal ) 
				{
					...
					{
						OspPrintf(TRUE, FALSE, "[ProcUserManagerMsg] GetMcuCurUserList Failed at index: %d\n", byUsrItr);
                        return;
					}
...
void CMcsSsnInst::ProcAddrBookMsg( const CMessage * pcMsg )
{
    if(NULL == g_pcAddrBook)
    {
        OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] g_pcAddrBook == NULL!\n");
        return;
    }
    ...
    case MCU_ADDRBOOK_GETENTRYLIST_NOTIF:    
        {
            ...
            if(0 == dwSize)
            {                                
                cServMsg.SetTotalPktNum(1);
                cServMsg.SetCurPktIdx(0);
                cServMsg.SetEventId(MCU_MCS_ADDRBOOK_GETENTRYLIST_NOTIF);
                SendMsgToMcs(MCU_MCS_ADDRBOOK_GETENTRYLIST_NOTIF, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] get first addr entry failed!\n");
                return;
            }
            ...
            {                    
                ...
                if(ADDRENTRYNUM_PERPKT == dwCurNum)
                {
                    cServMsg.SetCurPktIdx(byCurPktIdx++);
                    cServMsg.SetMsgBody((u8*)atAddrEntry, sizeof(TMcuAddrEntry)*dwCurNum);
                    SendMsgToMcs(MCU_MCS_ADDRBOOK_GETENTRYLIST_NOTIF, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                    dwCurNum = 0;
                }                
            }    
    ...
    case MCU_ADDRBOOK_GETGROUPLIST_NOTIF:
        {
            ...
            {                                
                cServMsg.SetTotalPktNum(1);
                cServMsg.SetCurPktIdx(0);
                cServMsg.SetEventId(MCU_MCS_ADDRBOOK_GETGROUPLIST_NOTIF);
                SendMsgToMcs(MCU_MCS_ADDRBOOK_GETGROUPLIST_NOTIF, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }     
            ...
            if(INVALID_INDEX == dwStartIdx)
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] get first addr group failed!\n");
                return;
            }
            ...
            {                    
                ...
                if(ADDRGROUPNUM_PERPKT == dwCurNum)
                {
                    cServMsg.SetCurPktIdx(byCurPktIdx++);
                    cServMsg.SetMsgBody((u8*)atAddrGroup, sizeof(TMcuAddrGroup)*dwCurNum);
                    SendMsgToMcs(MCU_MCS_ADDRBOOK_GETGROUPLIST_NOTIF, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                    dwCurNum = 0;
                }                
            }        
    ...
    case MCS_MCU_ADDRBOOK_ADDENTRY_REQ:
        {
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] addr entry is already exist\n");
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] add addr entry failed\n");
    ...
    case MCS_MCU_ADDRBOOK_DELENTRY_REQ:
        {
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] del addr entry failed\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
    ...
    case MCS_MCU_ADDRBOOK_MODIFYENTRY_REQ:
        {
            ...
            if ( !bRet )
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] modify addr entry failed\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
    ...
    case MCS_MCU_ADDRBOOK_ADDGROUP_REQ:
        {
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] addr group is already exist\n");
                cServMsg.SetErrorCode(ADDRBOOK_EXIST_ELEMENT);
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
            ...
            if ( INVALID_INDEX == dwIndex )
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] add addr group failed\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
    ...
    case MCS_MCU_ADDRBOOK_DELGROUP_REQ:
        {
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] del addr group failed\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
    ...
    case MCS_MCU_ADDRBOOK_MODIFYGROUP_REQ:
        {
            ...
            if ( !bRet )
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] modify addr group failed\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
    ...
    case MCS_MCU_ADDRBOOK_ADDENTRYTOGROUP_REQ:
        {
            ...
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] add addr to group failed, entry idx invalid\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }
            ...
            for ( u32 dwIdx = 0; dwIdx < dwAddrEntryNum; dwIdx++ )
            {
                ...
                {
                    OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] add addr to group failed, EntryNum :%d\n", dwAddrEntryNum);
                    cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                    SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                    return;
                }                
    ...
    case MCS_MCU_ADDRBOOK_DELENTRYFROMGROUP_REQ:
        {
            ...
            if(! g_pcAddrBook->IsAddrMultiSetEntryValid(dwGroupEntryIndex))
            {
                OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] del addr from group failed, entry idx invalid\n");
                cServMsg.SetErrorCode((u16)g_pcAddrBook->GetLastError());
                SendMsgToMcs(pcMsg->event+2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen());
                return;
            }        
    ...
    if(!g_pcAddrBook->SaveAddrBook(GetAddrBookPath()))          
    {
        OspPrintf(TRUE, FALSE, "[CMcsSsnInst][ProcAddrBookMsg] SaveAddrBook failed!\n");
    }
...
void CMcsSsnInst::AddrEntryOut2In(CAddrEntry *pcAddrIn, TMcuAddrEntry *ptAddrOut, u8 byNum)
{
    ...
    for(u8 byIdx = 0; byIdx < byNum; byIdx++)
    {
        ...
        ptAddrOut[byIdx].SetCallRate(ntohs(tInfo.wCallRate));
...
2006/2/18  4.0			许世林                  创建
=============================================================================*/
void CMcsSsnInst::AddrEntryIn2Out(TMcuAddrEntry *ptAddrIn, TADDRENTRY *ptAddrOut, u8 byNum)
{
    ...
    ptAddrOut->SetEntryInfo(&tAddrEntryInfo);
...
2006/2/18  4.0			许世林                  创建
=============================================================================*/
void CMcsSsnInst::AddrGroupIn2Out(TMcuAddrGroup *ptGroupIn, TADDRMULTISETENTRY *ptGroupOut, u8 byNum)
{
    ...
    u32 dwEntryNum = sizeof(adwEntryIdx);		
...
{        
    ...
    for (u8 byPackIdx = 0; byPackIdx < byPackNum; byPackIdx++)
    {
		...
		{
			OspPrintf(TRUE, FALSE, "[ProcNPlusUsrInfo] GetMcuCurUserList Failed at index: %d\n", byUsrItr);
            return;
		}
...
void CMcsSsnInst::ProcMcuDelUser( CMessage * const pcMsg )
{
    ...
    {
        // 如果是自己则断开和MCS的连接
        McsLog( "[ProcMcuDelUser] My Ssn User %s has been deleted, Disconnect with MCS!\n ", m_achUser );
