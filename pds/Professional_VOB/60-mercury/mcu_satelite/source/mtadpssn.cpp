...
void CMtAdpSsnInst::InstanceEntry( CMessage * const pcMsg )
{
	...
	if( NULL == pcMsg )
	{
		OspPrintf( TRUE, FALSE, "MtAdpSsn[InstanceEntry]: Null pointer message Received in InstanceEntry.\n" );
		return;
	}

	if( MT_MCU_MTSTATUS_NOTIF != pcMsg->event &&
        MCU_MCU_MTSTATUS_NOTIF != pcMsg->event)
	{
		MtLog( "MtAdpSsn[InstanceEntry]: %u(%s) passed!\n", pcMsg->event, ::OspEventDesc( pcMsg->event ) );
	}
...
//	case MCU_MT_REREGISTERGK_REQ:           //原有GK注册的323适配板掉线后，重新采用一块323适配板注册
	...
	default:
		OspPrintf( TRUE, FALSE, "MtAdpSsn[InstanceEntry]: Wrong message %u(%s) received in InstanceEntry()!\n", pcMsg->event, 
...
void CMtAdpSsnInst::DaemonInstanceEntry( CMessage * const pcMsg, CApp* pcApp )
{
	if (NULL == pcMsg)
	{
		OspPrintf( TRUE, FALSE, "MtAdpSsn[DaemonInstanceEntry]: The received msg's pointer in the msg entry is NULL!");
		return;
	}
	...
	case MCU_APPTASKTEST_REQ:			//GUARD Probe Message
		...
		break;
	default:
		OspPrintf( TRUE, FALSE, "MtAdpSsn[DaemonInstanceEntry]: Wrong message %u(%s) received!\n", pcMsg->event, 
			::OspEventDesc( pcMsg->event ) );
		break;
	}
...
{
    MtLog("[SendMsgToMtAdp]: Send Msg %u(%s) to MTAdpId%u Protocol%u MaxMtNum%u m_dwMtAdpAppIId.%d!\n", 
              wEvent, ::OspEventDesc(wEvent), m_byMtAdpId, m_byProtocolType, m_byMaxMtNum, m_dwMtAdpAppIId);
...
void CMtAdpSsnInst::ProcMtAdpRegisterReq( CMessage * const pcMsg )
{
	...
	//case STATE_NORMAL: 
        ...
        {
            OspPrintf( TRUE, FALSE, "CMtAdpSsnInst: invalid msg body len:%d, (should %d) nack.\n", 
		...
		{
			OspPrintf( TRUE, FALSE, "CMtAdpSsnInst: MtAdp's DriId: %d not equal to InsId: %d, nack.\n",tMtAdpReg.GetDriId(), GetInsID());
			post( pcMsg->srcid, pcMsg->event + 2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen(),  pcMsg->srcnode );
			::OspDisconnectTcpNode( pcMsg->srcnode );
			return;
		}
		...
		{
			OspPrintf( TRUE, FALSE, "CMtAdpSsnInst: MtAdp's ProtocolType: %d not support, nack.\n",tMtAdpReg.GetDriId() );
			post( pcMsg->srcid, pcMsg->event + 2, cServMsg.GetServMsg(), cServMsg.GetServMsgLen(),  pcMsg->srcnode );
			::OspDisconnectTcpNode( pcMsg->srcnode );
			return;
		}
		...
		{
			//NACK
			OspPrintf( TRUE, FALSE,"CMtAdpSsnInst: other mcu board mtadp[Ip= %0x, Id= %d] register while agent info[Ip = %0x], nack!\n", 
             ...
             tMtAdpReg.GetVersion() != DEVVER_MTADP )
        {
            ...
            OspPrintf(TRUE, FALSE, "MtadpSsn: Mtadp.%d(0x%x) version %d differs from MCU %d, NACK!\n",                       
                      GetInsID(),
                      tMtAdpReg.GetIpAddr(),
		...
		Mt2Log("[MtAdpSsn]: MtAdp %d register to mcu success!\n", tMtAdpReg.GetDriId() );
        ...
        {
            OspPrintf( TRUE, FALSE, "[MtAdpSsn]: get local cfg info failed!\n" );
        ...
        EqpLog("[MtAdpSsn: Reg Ack with System SSRC.%u.\n", dwSysSSrc);
		...
		if( 0 != g_cMcuAgent.GetGkIpAddr() && PROTOCOL_TYPE_H323 == m_byProtocolType)
		{
			...
			Mt2Log("CMtAdpSsnInst: MtAdp's DriId:%d Register To GK.\n", m_byMtAdpId);
		}        
        ...
        {
            OspPrintf(TRUE, FALSE, "[ProcEqpMcuRegMsg] get qos info failed!\n");
        }    
    ...
    default:
        OspPrintf(TRUE, FALSE, "MtAdpSsn: Wrong message %u(%s) received in current state %u!\n",
...
{
	OspPrintf( TRUE, FALSE, "[MtAdpSsn]: Mtadp disconnect messege %u(%s) received!,Dri id is:%d,ProtocolType is:%d,MaxMtNum is:%d.\n", 
...
void CMtAdpSsnInst::ProcMtAdpGetMsStatusReq( CMessage * const pcMsg )
{
    ...
    {
        OspPrintf(TRUE, FALSE, "[ProcMtAdpGetMsStatusReq]: Wrong message %u(%s) received in state.%u InsID.%u srcnode.%u!\n", 
    ...
    {        
        ...
        MtLog("[ProcMtAdpGetMsStatusReq]: IsMsSwitchOK :%d.\n", tMsStatus.IsMsSwitchOK());
    }
...
/*		if( MCU_MT_REREGISTERGK_REQ == pcMsg->event )
		{
				...
				0 == g_cMcuVcApp.GetRegGKDriId() && PROTOCOL_TYPE_H323 == m_byProtocolType )
			{
				...
				OspPrintf( TRUE, FALSE, "CMtAdpSsnInst: MtAdp's DriId:%d Register To GK Again.\n", m_byMtAdpId);
			}
		}
		else*/
...
void CMtAdpSsnInst::ProcMtAdpMcuGKMsg( CMessage * const pcMsg )
{
	...
	case STATE_NORMAL:
        ...
        {
            OspPrintf( TRUE, FALSE, "[Mtadpssn]: SrcDriId.(%d) != g_cMcuVcApp.GetRegGKDriId().(%d) %u(%s) return!\n", 
        ...
        {
            Mt2Log("[Mtadpssn]: ConfIdx<%d> unreg result.%d<%s>!\n", 
		...
		{
			Mt2Log("[Mtadpssn]: ConfIdx<%d> register to GK success!\n", byConfIdx );
		}
		else if( 0 == byConfIdx && MT_MCU_REGISTERGK_NACK == pcMsg->event )
		{
			Mt2Log("[Mtadpssn]: ConfIdx<%d> register to GK failure!\n", byConfIdx );
		}
			...
			{		
                ...
                if(tMapData.IsValidConf())
                {
                    if( NULL == g_cMcuVcApp.GetConfInstHandle( byConfIdx ) )
                    {
                        ...
                        OspPrintf( TRUE, FALSE, "[ProcMtAdpMcuGKMsg] RegAck for Conf.%d received while conf is not create yet which should be there before this moment, just take a try of its template once more\n", byConfIdx);

                        if(tMapData.IsTemUsed())
                        {
                            if(!g_cMcuVcApp.GetTemplate(byConfIdx, tTemInfo))
                            {
                                OspPrintf( TRUE, FALSE, "[ProcMtAdpMcuGKMsg] RegAck for Conf.%d whose template unexist yet, ignore it\n", byConfIdx);
                                return;
                            }
                            else
                            {
                                ptConfInfo = &tTemInfo.m_tConfInfo;
                                OspPrintf( TRUE, FALSE, "[ProcMtAdpMcuGKMsg] RegAck for Conf.%d whose template exist, continue\n", byConfIdx);
                            }
                        }
                        return;
                    }
                ...
                else if(tMapData.IsTemUsed())
                {
                    if(!g_cMcuVcApp.GetTemplate(byConfIdx, tTemInfo))
                    {
                        OspPrintf( TRUE, FALSE, "[ProcMtAdpMcuGKMsg] RegAck for Template.%d while template unexist, ignore it\n", byConfIdx);
                        return;
                    }
                    else
                    {
                ...
                {
                    OspPrintf( TRUE, FALSE, "[ProcMtAdpMcuGKMsg] RegAck for Conf/Temp.%d while both of them unexist, ignore it\n", byConfIdx);
                    return;
                }
...
BOOL32 CMtAdpConfig::SendMsgToMtAdpSsn( u8 byMpId, u16 wEvent, CServMsg & cServMsg )
{
	...
	{
		OspPrintf(TRUE, FALSE, "[MtAdpCfg]: SendMsgToMtAdpSsn Invalid MpId: %d.\n", byMpId);
		return FALSE;
	}
	else
	{
