...
void CMSManagerSsnInst::InstanceEntry( CMessage * const pcMsg )
{
	...
	case OSP_DISCONNECT: //上电通知
		ProcMcuMcuDisconnectNtf( pcMsg );
		break;

	case MCU_MSEXCHANGER_POWERON_CMD:
		ProcPowerOnCmd( pcMsg );
		break;
    ...
    case MCU_MCU_PROBE_REQ:
        ProcMcuMcuProbeReq( pcMsg );
        break;
        
    case MCU_MCU_PROBE_ACK:
        ProcMcuMcuProbeAck( pcMsg );
        break;
...
-----------------------------------------------------------------------------
    ...
    2005/10/18  4.0		    万春雷                  创建
=============================================================================*/
void CMSManagerSsnInst::ProcPowerOnCmd( CMessage * const pcMsg )
{
	...
	if (MCU_MSTYPE_SLAVE == g_cMSSsnApp.GetLocalMSType())
	{
        ...
        OspPrintf(TRUE, FALSE, "[ProcPowerOnCmd] start timer connect to another mcu.\n");
	}
...
void CMSManagerSsnInst::ClearInst( void )
{
	...
	OspPrintf(TRUE, FALSE, "[ClearInst] LocalMSInfo Type.%d State.%d\n", 
		         g_cMSSsnApp.GetLocalMSType(), g_cMSSsnApp.GetCurMSState());
...
-----------------------------------------------------------------------------
    ...
    2005/10/18  4.0		    万春雷                  创建
=============================================================================*/
void CMSManagerSsnInst::ProcMcuMcuDisconnectNtf( CMessage * const pcMsg )
{
	...
	if (FALSE == g_cMSSsnApp.GetMSDetermineType())
	{
		...
		if (MCU_MSSTATE_STANDBY == g_cMSSsnApp.GetCurMSState())
		{
			...
			OspPrintf(TRUE, FALSE, "[McuMcuDisconnectNtf] STANDBY->ACTIVE, IsMsSwitchOK :%d\n",
	...
	if (MCU_MSTYPE_SLAVE == g_cMSSsnApp.GetLocalMSType())
	{
        ...
        OspPrintf(TRUE, FALSE, "[ProcMcuMcuDisconnectNtf] start timer connect to another mcu.\n");
    }
...
void CMSManagerSsnInst::ProcMcuMcuRegisterReq( CMessage * const pcMsg )
{
	...
	{
		post( pcMsg->srcid, MCU_MCU_REGISTER_NACK, NULL, 0, pcMsg->srcnode );
		OspDisconnectTcpNode( pcMsg->srcid );
		ClearInst();
...
2006/4/4  4.0			许世林                  创建
=============================================================================*/
void CMSManagerSsnInst::ProcMcuMcuProbeReq( CMessage * const pcMsg )
{
...
2006/4/4  4.0			许世林                  创建
=============================================================================*/
void CMSManagerSsnInst::ProcMcuMcuProbeAck( CMessage * const pcMsg )
{
...
-----------------------------------------------------------------------------
    ...
    2005/10/18  4.0		    万春雷                  创建
=============================================================================*/
void CMSManagerSsnInst::ProcMcuMcuEndSynNtf( CMessage * const pcMsg )
{
	if (MS_STATE_NORMAL != CurState() || 
		MCU_MSSTATE_STANDBY != g_cMSSsnApp.GetCurMSState())
	{
	...
	PrintMSInfo("[ProcMcuMcuEndSynNtf] EndSyn, CurTicks :%u\n", OspTickGet());
...
void CMSManagerSsnInst::ProcMcuMcuSynningDataReq( CMessage * const pcMsg )
{
	...
	PrintMSVerbose("[ProcMcuMcuSynningDataReq] RecvCustomMsgFromOtherMcu OK: MsgID.%d FrmLen.%u FrmSN.%d\n", 
...
void CMSManagerSsnInst::ProcMcuMcuSynningDataRsp( CMessage * const pcMsg )
{
	...
	PrintMSInfo("[ProcMcuMcuSynningDataRsp] RetVal.%d MsgID.%d FrmLen.%u FrmSN.%d\n", 
	...
	if ((u8)emMSReturnValue_Ok != ptMSDataRsp->m_byRetVal)
	{
		...
		PrintMSException("[ProcMcuMcuSynningDataRsp] RetVal.%d!\n", ptMSDataRsp->m_byRetVal);
		return;
	}
...
{
	u32 dwOutBufLen = 0;
	if (FALSE == g_cMcuVcApp.GetVcDeamonOtherData(m_tSndFrmHead.m_pbyFrmBuf, 
		                                          MS_MAX_FRAME_LEN, dwOutBufLen))
	{
...
BOOL32 CMSManagerSsnInst::SaveSynDataOfVcDeamonOtherData( u8 *pbyBuf, u32 dwBufLen )
{
	...
	{
		bRet = g_cMcuVcApp.SetVcDeamonOtherData( pbyBuf, dwBufLen );
		if (FALSE == bRet)
		{
...
void CMSManagerSsnInst::MSManagerPrint( u8 byLevel, s8 *pszPrintStr )
{
	...
	if( byLevel > g_cMSSsnApp.GetDebugLevel() )
		return;
...
BOOL32 CMSManagerConfig::InitMSConfig( TMSStartParam tMSStartParam )
{
	...
	if (m_bDoubleLink)
	{
		...
		g_cMSSsnApp.CreateApp( "McuMSMag", AID_MCU_MSMANAGERSSN, APPPRI_MCUMSMGR, 1000, 4<<20 );
...
BOOL32 CMSManagerConfig::InitLocalCfgInfo( u8 byLocalMcuType )
{
	...
	//alloc memory
	ppszTable = new s8*[nEntryNum];
	if (NULL == ppszTable)
	{
		printf("[InitLocalCfgInfo] Fail to malloc memory for board table\n");
		return FALSE;
	}
	memset(ppszTable, 0, nEntryNum*sizeof(s8*));
	...
	for (nLoop = 0; nLoop < nEntryNum; nLoop++ )
	{
		...
		if (DSL8000_BRD_MPC == byEntType)
		{
			...
			else if( tCurrentPos.byBrdSlot == byEntSlot ) // 配置文件中MpcIp有误
			{
				...
				OspPrintf(TRUE, FALSE, "[!!Debug!!] the mpc's ip %0x.\n", m_dwLocalMcuIP);
			}
#endif
			else
			{
	...
	if (0 == m_dwLocalMcuIP)
	{
		OspPrintf(TRUE, FALSE, "[InitLocalCfgInfo] Wrong profile while LocalMcuIP!\n");
		m_bDoubleLink   = FALSE;
		m_dwRemoteMcuIP = 0;
		return FALSE;
	}
...
void CMSManagerConfig::SetCurMSState(emMCUMSState emCurMSState)
{
	...
	{
		OspPrintf(TRUE, FALSE, "[SetCurMSState] invalid argument emCurMSState(%d), Failed!\n", emCurMSState);
		return;
	}
	if (emCurMSState != m_emCurMSState)
	{
		...
		if (FALSE == m_bMSDetermineType && TRUE == m_bDoubleLink)
		{
			...
			{
				OspPrintf(TRUE, FALSE, "[SetCurMSState] BrdMPCSetLocalMSState Failed Old_CurMSState.%d New_CurMSState.%d!\n", 
	...
	if (m_bInited)
	{
		g_cMcuAgent.SetMpcEnabled((MCU_MSSTATE_ACTIVE == m_emCurMSState));
	}
...
BOOL32 CMSManagerConfig::GetTypeThroughName( const s8* pszName, u8* pbyType )
{
	...
	{
		*pbyType = BRD_HWID_KDM200_HDU;
		return TRUE;
	}
	else if ( 0 == strcmp( pszName, "MPU") )
	{
		*pbyType = BRD_HWID_DSL8000_MPU;
		return TRUE;
	}
...
API void showmsstate()
{
	...
	{
		memcpy(achPrt, "KDV8000A", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
	else if (MCU_TYPE_KDV8000B == byLocalMcuType)
	{
		memcpy(achPrt, "KDV8000B", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
	else if (MCU_TYPE_WIN32 == byLocalMcuType)
	{
		memcpy(achPrt, "KDVWIN32", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
    else if (MCU_TYPE_KDV8000C == byLocalMcuType)
    {
        memcpy(achPrt, "8000C", sizeof(achPrt));
        achPrt[MS_MAX_STRING_LEN-1] = '\0';
    }    
	else
	{
		memcpy(achPrt, "UNKNOWN", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
	
	OspPrintf(TRUE, FALSE, "MS Base State: LocalMcuIP:0x%0x RemoteMcuIP:0x%0x McuType:%s DoubleLink:%d Connected: %d SysSSRC:%u\n", 
		      dwLocalMcuIP, dwRemoteMcuIP, achPrt, bDoubleLink, bConnected, dwSysSSrc);

	memset(achPrt, 0, sizeof(achPrt));
	if (MCU_MSTYPE_MASTER == emLocalMSType)
	{
		memcpy(achPrt, "MC1(MST)", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
	else if (MCU_MSTYPE_SLAVE == emLocalMSType)
	{
		memcpy(achPrt, "MC0(SLV)", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
	else
	{
		memcpy(achPrt, "INVALID", sizeof(achPrt));
		achPrt[MS_MAX_STRING_LEN-1] = '\0';
	}
	...
	if (MCU_MSSTATE_ACTIVE == emCurMSState)
	{
		memcpy(achPrt2, "ACTIVE", sizeof(achPrt2));
		achPrt2[MS_MAX_STRING_LEN-1] = '\0';
	}
	else if (MCU_MSSTATE_STANDBY == emCurMSState)
	{
		memcpy(achPrt2, "STANDBY", sizeof(achPrt2));
		achPrt2[MS_MAX_STRING_LEN-1] = '\0';
	}
	else
	{
		memcpy(achPrt2, "OFFLINE", sizeof(achPrt2));
		achPrt2[MS_MAX_STRING_LEN-1] = '\0';
	}
	
	OspPrintf(TRUE, FALSE, "MS Comm State: MSType:%s MSState:%s Inited:%d SynOK:%d SwitchOK:%d MSDetermineType:%d HWDitheringTimes:%u\n", 
