...
void CMcuVcInst::DaemonProcMcuDcsConnectedNtf( const CMessage * pcMsg )
{
	...
	{
		DcsLog( "[DcsNtf] the DCS id should not be NULL, ignore it !\n");
		return;
	}
	...
	if ( TRUE == bNoSpace )
	{
		DcsLog( "[DcsNtf] there is no space for new DCS.%d which is impossible, check it !\n", byDcsId );
		return;
	}
	...
	{
		ConfLog( FALSE, "Dcs %d register failed due to no config !\n", byDcsId );
		return;
	}
...
/*
	...
	if ( FALSE == bValid )
	{
		DcsLog( "get dcs ip: %s from agent failed !\n", tDcsData.m_dwDcsIp );
		return;
	}
...
*/
    ...
    {
        CServMsg cServMsg;
        cServMsg.SetEventId(MCU_MCS_MCUPERIDCSSTATUS_NOTIF);
        cServMsg.SetMsgBody( (u8*)&tDcsStatus, sizeof(TPeriDcsStatus) );
    ...
    ConfLog( FALSE, "Dcs 0x%x register success !\n", tDcsInfo.m_dwDcsIp );
    return;
...
void CMcuVcInst::DaemonProcDcsMcuReleaseConfRsp( const CMessage * pcMsg )
{
	...
	if ( NULL == pcMsg )
	{
		DcsLog( "get release conf rsp msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs release conf rsp msg: %d<%s> received in inst %d !\n", 
	...
	case DCSSSN_MCU_RELEASECONF_NACK:
		{
			...
			DcsLog( "release data conf %d failed due to reason: %d !\n", byConfIdx, byErrorId );
	...
	case DCSSSN_MCU_CONFRELEASED_NOTIF:
		{
			...
			ConfLog( FALSE, "Data conf %d released succeed on Dcs !\n", byConfIdx );
	...
	default:
		DcsLog( "unexpected dcs release conf rsp msg received, ignore it !\n");
		break;
	}
}
...
{	
	...
	case CONF_DATAMODE_VAONLY:
		DcsLog( "Conf data mode should not be va only !\n" );
		break;
	...
	case CONF_DATAMODE_VAANDDATA:	
		{		
			...
			{
				ConfLog( FALSE, "data conf: %s created failed due to no DCS registered yet !\n", m_tConf.GetConfName() );
			...
			else
			{
				...
				if ( 0 == wBandWidth )
				{
					DcsLog( "get dsc data conf bitrate failed !\n");
					return;
				}
				...
				DcsLog( "send to dcsssn create conf %s req success !\n", m_tConf.GetConfName() );
			}
		}
		break;
	...
	case CONF_DATAMODE_DATAONLY:
		DcsLog( "Current system couldn't support data only conf !\n");
		break;
		
	default:
		DcsLog( "unexpected data conf mode: %d, ignore it!\n", m_tConf.GetConfAttrb().GetDataMode() );
		break;
	}
...
void CMcuVcInst::ProcDcsMcuCreateConfRsp( const CMessage * pcMsg )
{
	...
	{
		DcsLog( "wrong conf state in create conf rsp!\n" );
		return;
	}
	//消息检测
	if ( NULL == pcMsg )
	{
		DcsLog( "get create conf rsp msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs create conf rsp msg: %d<%s> received in inst %d !\n", 
			pcMsg->event, OspEventDesc(pcMsg->event), GetInsID() );
	}	
	//消息处理
	CServMsg cServMsg( pcMsg->content, pcMsg->length );
	...
	case DCSSSN_MCU_CREATECONF_ACK:
		{
			...
			{
				DcsLog( "wrong confidx %d in create Data conf ack should be %d !\n", 
	...
	case DCSSSN_MCU_CREATECONF_NACK:
		{
			...
			if ( m_byConfIdx != cServMsg.GetConfIdx() )
			{
				DcsLog( "wrong confidx %d in create Data conf Nack should be %d !\n", 
					       cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//错误类型
			...
			{
				DcsLog( "create data conf %s failed due reason: %d", m_tConf.GetConfName(), byErrorId );
			}
			else
			{
				m_tConfInStatus.SetDataConfRecreated(FALSE);
				DcsLog( "Recreate data conf %s failed due reason: %d", m_tConf.GetConfName(), byErrorId );
			}
	...
	case DCSSSN_MCU_CONFCREATED_NOTIF:
		{
			...
			{
				DcsLog( "wrong confidx %d in create Data conf Notify should be %d !\n", 
						   cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//提取数据会议信息
			...
			if ( 0 !=dwRet )
			{
				DcsLog( "wrong conf name %s in create Data conf Notify should be %s !\n",
			...
			if ( NULL != m_tConf.GetConfPwd() ) 
			{
				u32 dwRet = memcmp( tDcsConfInfo.GetConfPwd(), m_tConf.GetConfPwd(), sizeof(u8) * MAXLEN_PWD );
				//不匹配
				if ( 0 != dwRet )
				{
					DcsLog( "wrong conf pwd %d in create conf Notify should be %s !\n",
			...
			if ( 0 != tDcsConfInfo.GetBandwidth() )
			{
				...
				if ( 64 < m_tConf.GetDcsBitRate() ) 
				{
					...
					{
						DcsLog( "wrong conf bandwidth %d in create conf Notify should be %d !\n", 
				...
				else	
				{
					...
					{
						DcsLog( "wrong conf bandwidth %d in create conf Notify should be 64 !\n", 
			...
			*/
			...
			if ( m_tConfInStatus.IsDataConfRecreated() )
			{
				...
				ConfLog( FALSE, "Data Conf %s Recreated and started on Dcs %d belong to Mcu Inst %d !\n",
					 m_tConf.GetConfName(), m_byDcsIdx, GetInsID() );
			}
			else
			{
				ConfLog( FALSE, "Data Conf %s created and started on Dcs %d belong to Mcu Inst %d !\n",
					 m_tConf.GetConfName(), m_byDcsIdx, GetInsID() );
			}
	...
	default:
		DcsLog( "unexpected dcs create conf rsp msg %d<%s> received !\n", pcMsg->event, OspEventDesc(pcMsg->event) );
		break;
	}
}
...
void CMcuVcInst::SendMcuDcsAddMtReq( u8 byMtId, u32 dwMtIp, u16 wPort )
{
	...
	if ( STATE_ONGOING != CurState() )
	{
		DcsLog( "wrong conf state in del mt req !\n" );
		return;
	}	
	//本端的DCS ip检测
	if ( 0 == g_cMcuVcApp.m_atPeriDcsTable[m_byDcsIdx - 1].m_dwDcsIp ) 
	{
		DcsLog( "Get current dcs Ip failed !\n");
		return;
	}
	//DCS状态
	if ( !g_cMcuVcApp.IsPeriDcsValid(m_byDcsIdx) || !g_cMcuVcApp.IsPeriDcsConnected(m_byDcsIdx) )
	...
	if ( m_ptMtTable->IsNotInvited( byMtId ) )
	{
		...
		if ( !m_ptMtTable->IsMtIsMaster( byMtId ) )
		{
			...
			DcsLog( "mcu data conf %s add mt%d as master request send out !\n", m_tConf.GetConfName(), byMtId );
		}
		//本端做从
		else
		{
			...
			DcsLog( "mcu join mt%d's data conf %s as slave request send out !\n", byMtId, m_tConf.GetConfName() );
		}
	...
	else
	{
		...
		DcsLog( "mcu data conf %s invite mt%d as master request send out !\n", m_tConf.GetConfName(), byMtId );
	}
...
void CMcuVcInst::ProcDcsMcuAddMtRsp( const CMessage * pcMsg )
{
	...
	if ( STATE_ONGOING != CurState() )
	{
		DcsLog( "wrong conf state in add mt rsp!\n" );
		return;
	}
	//消息检测
	if ( NULL == pcMsg )
	{
		DcsLog( "get add mt rsp msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs add mt rsp msg: %d<%s> received in inst %d !\n", 
			pcMsg->event, OspEventDesc(pcMsg->event), GetInsID() );
	}
	//消息处理
	CServMsg cServMsg( pcMsg->content, pcMsg->length );
	...
	case DCSSSN_MCU_ADDMT_ACK:
		{
			...
			if ( cServMsg.GetConfIdx() != m_byConfIdx )
			{
				DcsLog( "wrong confidx %d received in add mt ack should be %d", 
						 cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//终端ID
			...
			if ( 0 == byMtId )
			{
				DcsLog( "wrong mt id %d received in add mt ack !\n", byMtId );
				return;
			}
	...
	case DCSSSN_MCU_ADDMT_NACK:
		{
			...
			if ( cServMsg.GetConfIdx() != m_byConfIdx )
			{
				DcsLog( "wrong confidx %d received in add mt Nack should be %d\n", 
						   cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//终端ID
			...
			if ( 0 == byMtId )
			{
				DcsLog( "wrong mt id %d received in add mt nack !\n", byMtId );
				return;
			}
			//错误类型
			u8 byErrorId = *(cServMsg.GetMsgBody() + sizeof(u8));
			DcsLog( "conf %s add mt%d failed due to reason: %d \n", 
	...
	default:
		DcsLog( "unexpected msg %d<%s> received in add mt rsp 1\n", pcMsg->event, OspEventDesc(pcMsg->event) );
		break;
	}
...
void CMcuVcInst::SendMcuDcsDelMtReq( u8 byMtId )
{
	...
	if ( !g_cMcuVcApp.IsPeriDcsValid(m_byDcsIdx) || !g_cMcuVcApp.IsPeriDcsConnected(m_byDcsIdx) )
	{
		DcsLog( "dcs %d not exist, del data mt%d failed !\n", m_byDcsIdx, byMtId );
		return;
	}
...
void CMcuVcInst::ProcDcsMcuDelMtRsp( const CMessage * pcMsg )
{
	...
	if ( STATE_ONGOING != CurState() )
	{
		DcsLog( "wrong conf state in del mt rsp!\n" );
		return;
	}
	//消息检测
	if ( NULL == pcMsg )
	{
		DcsLog( "get del mt rsp msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs del mt rsp msg: %d<%s> received in inst %d !\n", 
			       pcMsg->event, OspEventDesc(pcMsg->event), GetInsID() );
	}
	//消息处理
	CServMsg cServMsg( pcMsg->content, pcMsg->length );
	...
	case DCSSSN_MCU_DELMT_ACK:
		{
			...
			if ( cServMsg.GetConfIdx() != m_byConfIdx )
			{
				DcsLog( "wrong confidx %d received in del mt ack should be %d !\n", 
						 cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//终端ID
			...
			if ( 0 == byMtId )
			{
				DcsLog( "wrong mt id %d received in del mt ack !\n", byMtId );
				return;
			}
	...
	case DCSSSN_MCU_DELMT_NACK:
		{
			...
			if ( cServMsg.GetConfIdx() != m_byConfIdx )
			{
				DcsLog( "wrong confidx %d received in del mt Nack should be %d !\n", 
						   cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//终端ID
			...
			if ( 0 == byMtId )
			{
				DcsLog( "wrong mt id %d received in del mt Nack !\n", byMtId );
				return;
			}
			//错误类型
			u8 byErrorId = *(cServMsg.GetMsgBody() + sizeof(u8) );
			DcsLog( "Conf %s del mt%d failed due to reason: %d !\n", m_tConf.GetConfName(), byErrorId );
	...
	default:
		DcsLog( "unexpected msg %d<%s> received in del mt rsp !", pcMsg->event, OspEventDesc(pcMsg->event) );
		break;
	}
...
void CMcuVcInst::ProcDcsMcuMtJoinedNtf( const CMessage * pcMsg )
{
	...
	if ( STATE_ONGOING != CurState() )
	{
		DcsLog( "wrong conf state in mt joined notify !\n" );
		return;
	}
	//消息检测
	if ( NULL == pcMsg )
	{
		DcsLog( "get mt joined notify msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs mt joined notify msg: %d<%s> received in inst %d !\n", 
			pcMsg->event, OspEventDesc(pcMsg->event), GetInsID() );
	}
	//消息处理
	CServMsg cServMsg( pcMsg->content, pcMsg->length );
	...
	case DCSSSN_MCU_MTJOINED_NOTIF:
		{
			...
			if ( m_byConfIdx != cServMsg.GetConfIdx() )
			{
				DcsLog( "wrong confidx: %d received in mt joined conf notify should be %d \n",
						 cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//终端ID
			...
			if ( 0 == byMtId )
			{
				DcsLog( "wrong mt id %d received in mt joined conf notify !\n");
				return;
			}
			...
			if ( TDcsMtInfo::emT120Mcu == tDcsMtInfo.m_byDeviceType )
			{
				...
				if ( m_ptMtTable->IsMtIsMaster(byMtId) )
				{
					ConfLog( FALSE, "mt%d joined data conf %s as Master_Dcs !\n", byMtId, m_tConf.GetConfName() );
				}
				//本端做主, 下级MCU加入本端
				else
				{
					ConfLog( FALSE, "mt%d joined data conf %s as Slave_Dcs !\n", byMtId, m_tConf.GetConfName() );
				}
			}
			//上线终端是MT
			else
			{
				ConfLog( FALSE, "mt%d joined data conf %s as a general DCMT !\n", byMtId, m_tConf.GetConfName() );
			}
			...
			{
				DcsLog( "Mt%d should not in data conf %s already !\n", byMtId, m_tConf.GetConfName() );
				return;
			}
	...
	default:
		DcsLog( "unexpected msg %d<%s> received in mt joined notify !\n" );
		break;
	}
}
...
void CMcuVcInst::ProcDcsMcuMtLeftNtf( const CMessage * pcMsg )
{
	...
	if ( STATE_ONGOING != CurState() )
	{
		DcsLog( "wrong conf state in mt left notify !\n" );
		return;
	}
	//消息检测
	if ( NULL == pcMsg )
	{
		DcsLog( "get mt left notify msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs mt left notify msg: %d<%s> received in inst %d !\n", 
			pcMsg->event, OspEventDesc(pcMsg->event), GetInsID() );
	}
	//消息处理
	CServMsg cServMsg( pcMsg->content, pcMsg->length );
	...
	case DCSSSN_MCU_MTLEFT_NOTIF:
		{
			...
			if ( m_byConfIdx != cServMsg.GetConfIdx() )
			{
				DcsLog( "wrong confidx: %d received in mt left conf notify should be %d \n",
						 cServMsg.GetConfIdx(), m_byConfIdx );
				return;
			}
			//终端ID
			...
			if ( 0 == byMtId )
			{
				DcsLog( "wrong mt id %d received in mt left conf notify !\n");
				return;
			}			
			//掉线终端的类型: 后续扩展 ?
			...
			ConfLog( FALSE, "Mt%d left Data Conf %s belong to MCU Inst %d !\n", byMtId, m_tConf.GetConfName(), GetInsID() );
	...
	default:
		DcsLog( "unexpected msg %d<%s> received in mt left notify !\n", pcMsg->event, OspEventDesc(pcMsg->event) );
		break;
	}
}
...
void CMcuVcInst::ProcDcsMcuStatusNotif( const CMessage * pcMsg )
{
	...
	if ( STATE_ONGOING != CurState() )
	{
		DcsLog( "wrong conf state in dcs disconnect notify !\n" );
		return;
	}
	//消息检测
	if ( NULL == pcMsg )
	{
		DcsLog( "get dcs status notify msg failed! \n");
		return;
	}
	else
	{
		DcsLog( "dcs status notify msg: %d<%s> received in inst %d !\n", 
			pcMsg->event, OspEventDesc(pcMsg->event), GetInsID() );
	}
	//消息处理
	CServMsg cServMsg( pcMsg->content, pcMsg->length );
	...
	default:
		DcsLog( "unexpected dcs status notify msg %d<%s> received !\n", pcMsg->event, OspEventDesc(pcMsg->event) );
		break;
	}
}
...
=============================================================================*/
void CMcuVcInst::ProcTimerRecreateDataConf( const CMessage * pcMsg )
{
	if ( FALSE == FindOnlineDcs() )
	{
		ConfLog( FALSE, "There is no DCS online still, recreate data conf %s failed !\n", m_tConf.GetConfName() );
