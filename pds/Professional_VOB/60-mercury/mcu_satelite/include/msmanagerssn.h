...
#define MS_DEBUG_CRITICAL           (u8)0
#define MS_DEBUG_ERROR              (u8)1
#define MS_DEBUG_WARNING            (u8)2
#define MS_DEBUG_INFO               (u8)3
#define MS_DEBUG_VERBOSE            (u8)4
...
struct TMSVcConfState
{
	...
	u16 m_awConfDataLen[MAXNUM_MCU_CONF+MAXNUM_MCU_TEMPLATE+1]; //会议数据长度，网络序
}
#ifndef WIN32
__attribute__ ( (packed) ) 
#endif
...
{
	u8 m_abyOnline[MAXNUM_SUB_MCU]; //1-online
}
#ifndef WIN32
__attribute__ ( (packed) ) 
#endif
...
protected:
	...
	void ProcPowerOnCmd( CMessage * const pcMsg );
	void ClearInst( void );
	...
	//MCU之间 断链通知
	void ProcMcuMcuDisconnectNtf( CMessage * const pcMsg );	
	...
	void ProcMcuMcuEndSynNtf( CMessage * const pcMsg );
    ...
    void ProcMcuMcuProbeReq( CMessage * const pcMsg );
    void ProcMcuMcuProbeAck( CMessage * const pcMsg );
