...
BOOL CBoardAgent::GetBoardCfgInfo()
{
    ...
    if( FALSE == bResult )  
	{
		OspPrintf( TRUE, TRUE, "[Cri] Wrong profile while reading %s!\n", "BoardIpAddr" );
...
BOOL CBoardAgent::ConnectManager(u32& dwMcuNode, u32 dwMpcIp, u16 wMpcPort)
{
    ...
    if( !OspIsValidTcpNode(dwMcuNode) )  // 未建立连接
    {
        ...
        {
            OspPrintf(TRUE, FALSE, "[CriAgent] CreateTcpNode with Manager Failed!\n");
            bRet = FALSE;
        }
        else
        {
            ...
            brdagtlog( "[Cri] connect to mcu success, dwMcuNode:%d, wDiscHeartBeatTime:%d, byDiscHeartBeatNum:%d\n",
...
void CBoardAgent::ProcBoardPowerOn( CMessage* const pcMsg )
{
...
#ifndef WIN32	
        BrdQueryPosition( &m_tBoardPosition ); // 查询单板位置
        TBrdEthParam tBrdEthParam;          
        BrdGetEthParam(g_cBrdAgentApp.m_byChoice, &tBrdEthParam);// 取单板Ip
        m_dwBoardIpAddr = tBrdEthParam.dwIpAdrs;  // 网络序 
#endif
	...
	default:
		OspPrintf(TRUE, FALSE, "[Cri] Wrong state %u when in (ProcBoardPowerOn)!\n", CurState() );
		break;
	}
	return;
}
...
void CBoardAgent::ProcBoardRegNAck( CMessage* const pcMsg )
{
	...
	default:
		OspPrintf(TRUE, FALSE, "BoardAgent: Wrong state %d when receive RegNack!\n", 
			CurState() );
		break;
	}
	return;
...
void CBoardAgent::MsgRegAck(CMessage* const pcMsg )
{
    if(NULL == pcMsg)
    {
        OspPrintf(TRUE, FALSE, "[Dri] the pointer can not be Null.(MsgRegAck)\n");
    }
...
void CBoardAgent::ProcGetConfigTimeOut( CMessage* const pcMsg )
{
	...
	default:
		OspPrintf(TRUE, FALSE, "BoardAgent: Wrong state %u when receive get configure!\n", 
			CurState() );
		break;
	}
	return;
...
void CBoardAgent::ProcBoardGetConfigNAck( CMessage* const pcMsg )
{
	...
	default:
		OspPrintf(TRUE, FALSE,  "BoardAgent: Wrong state %u when receive get config Nack!\n", 
			CurState() );
		break;
	}
	return;
    ...
    // case STATE_NORMAL:  // test ..
		...
		{			
			...
			case EQP_TYPE_PRS:
                ...
                brdagtlog( "[Cri] The Cri board run the Prs module\n");
				break;
		...
		// 以后是单板IP        
        brdagtlog( "BoardAgent: g_cBrdAgentApp.SetBrdIpAddr(0x%x)-wIndex = %d\n",
...
void CBoardAgent::ProcLedStatusReq( CMessage* const pcMsg )
{
	...
	case STATE_NORMAL:
        ...
        memcpy(&m_tLedState, &tBrdLedState.nlunion.tBrdCRILedState, sizeof(TBrdCRILedStateDesc));
...
BOOL CBoardAgent::PostMsgToManager( u16 wEvent, u8 * const pbyContent, u16 wLen )
{
...
#ifdef _DEBUG
		OspPrintf(TRUE, FALSE, "BoardAgent: Post message %u(%s) to Board manager A Error!\n", wEvent, 
			::OspEventDesc( wEvent ), m_tBoardPosition.byBrdID, m_tBoardPosition.byBrdLayer,
	...
	{
		OspPrintf(TRUE, FALSE, "BoardAgent: Board [%d,%d,%d] is offline now: Node=%u, CurState=%u, InstId=%u\n", 
			m_tBoardPosition.byBrdID, m_tBoardPosition.byBrdLayer, 
            m_tBoardPosition.byBrdSlot, m_dwDstNode, CurState(), GetInsID() );
	}
    ...
    {
		brdagtlog( "[Cri] the B's node is invalide\n");
...
void CBoardAgent::ProcBoardLedStatus( CMessage* const pcMsg )
{
	...
	ptLedState = (TBrdCRILedStateDesc*)pcMsg->content;
