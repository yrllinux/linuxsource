...
BOOL CBoardAgent::GetBoardCfgInfo()
{
    ...
    m_dwBoardIpAddr = inet_addr( achReturn );  // 网络序
...
BOOL CBoardAgent::ConnectManager(u32& dwMcuNode, u32 dwMpcIp, u16 wMpcPort)
{
    ...
    {
        dwMcuNode = OspConnectTcpNode( htonl(dwMpcIp), wMpcPort, 10, 3, 1000);// 连接Mcu 代理
...
void CBoardAgent::ProcBoardRegAck( CMessage* const pcMsg )
{
    ...
    case STATE_NORMAL:
		if( pcMsg->srcnode == m_dwDstNode )
        {
	    	m_dwDstIId = pcMsg->srcid; // 记录对方实例号
    ...
    if(STATE_INIT == CurState())
    {
        PostGetCfgMsg( BOARD_MPC_GET_CONFIG, 
            (u8*)&m_tBoardPosition, sizeof(m_tBoardPosition) ); //重新发送取配置信息消息
...
{
	PostGetCfgMsg( BOARD_MPC_GET_CONFIG, (u8*)&m_tBoardPosition, sizeof(m_tBoardPosition) ); //重新发送取配置信息消息
	return;
}
...
void CBoardAgent::ProcOspDisconnect( CMessage* const pcMsg )
{
    ...
    #ifndef WIN32
    #ifdef MMP
		    BrdLedStatusSet( LED_SYS_LINK, BRD_LED_OFF ); // 指示灯关闭
    #endif
    #endif
...
/*			
			struct tm   *ptTimeA;
			ptTimeA = localtime(&m_tLastDisTimeA); // 转换为标准时间
			struct tm   *ptTimeB;
