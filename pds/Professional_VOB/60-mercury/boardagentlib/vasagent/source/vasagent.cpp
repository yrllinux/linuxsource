...
BOOL CBoardAgent::ConnectManager(u32& dwMcuNode, u32 dwMpcIp, u16 wMpcPort)
{
    ...
    if( !OspIsValidTcpNode(dwMcuNode) )  // 未建立连接
    {
        ...
        {
            ::OspNodeDiscCBReg( dwMcuNode, GetAppID(), GetInsID() );	//注册断链接收实例
        }
    }
	return bRet;
}
...
void CBoardAgent::ProcBoardPowerOn( CMessage* const pcMsg )
{
	...
	case STATE_IDLE:
        if(0 != m_dwMpcIpAddr)
        {
            bRet = ConnectManager(m_dwDstNode, m_dwMpcIpAddr, m_wMpcPort);  //和MPC上的管理程序连接
            if( TRUE == bRet)
            {
        ...
        if(0 != m_dwMpcIpAddrB)
        {
            bRet = ConnectManager(m_dwDstNodeB, m_dwMpcIpAddrB, m_wMpcPortB);  //和MPC上的管理程序连接
            if( TRUE == bRet)
            {
