...
#define LOG_PRTP						(u8)6  //仅对RTP LOG有效，不需要调用setmplog 放开
...
#define RTCPBACK_INTERVAL      (u8)30

#define  MAX_AUDIO_FRAME_SIZE           (u32)2<<10
...
struct PinHoleInfo
{
    u16 m_wLocalPort;			//本地打洞端口
    u32 m_dwRemoteIp;			//远端打洞地址
    u16 m_wRemotePort;			//远端打洞端口
    EMGwPinholeType m_emType;	//打洞类型RTP/RTCP
    u8	m_byPlayLoad;			//通道载荷	    
...
{
public:
	//lint -sem(CNetTrfcStat::Reset,initializer)
    CNetTrfcStat();
    ~CNetTrfcStat();
...
public:
    ...
    void RegisterToMcu(u32 dwMcuNode);		//向Mcu注册
	...
	void ProcAddSendSelfMutePackCmd(CMessage* const pcMsg);
	void ProcDelSendSelfMutePackCmd(CMessage* const pcMsg);

    void ProcMcuMpPinHoleNotify(CMessage * const pcMsg);		//打洞消息处理
	void PorcMcuMpStopPinHoleNotify(CMessage * const pcMsg);	//补洞消息处理
	...
	void SendSelfMutePack(u32 dwNeedDstIp,u16 wNeedDstPort);

protected:
    void SendMsgToMcu(u16 wEvent, u8 * const pbyMsg = NULL, u16 wLen = 0);
    void SendMsg2Mcu(u16 wEvent, CServMsg &cServMsg);
	CKdvMediaSnd * m_ppSnd[MEDIA_TYPE_NULL] ;//一种meidatype只用一个发送对象
	u8  m_achBuf[MAX_AUDIO_FRAME_SIZE];
};

#define MAXnum_ReverseG7221cPort			100 //定义内部使用最大记录g7221c数据翻转个数
...
public:
	...
	void   AddReverseG7221cPort( u16 wRcvPort );		//增加需要对g7221.c音频格式码流内容奇偶对换的端口
	BOOL32 IsPortNeedReverseG7221cData( u16 wRcvPort );	//该端口是否需要对g7221.c音频格式码流内容奇偶对换
	void   RemoveReverseG7221cPort( u16 wRcvPort );		//清除需要对g7221.c音频格式码流内容奇偶对换的端口
    ...
    //设置打洞间隔
    void SetPinHoleInterval(u32 dwPinHoleInterval){m_dwPinHoleInterval = dwPinHoleInterval;}
    //获取打洞间隔
    u32  GetPinHoleInterval(void){return m_dwPinHoleInterval;}
    //添加打洞信息
    void SetPinHoleTable(PinHoleInfo atPinHoleTable, u16 wIndex){m_atPinHoleTable[wIndex] = atPinHoleTable;}
    //获取打洞信息
    PinHoleInfo GetPinHoleItem(u16 wIndex){ return m_atPinHoleTable[wIndex]; }
	
	//设置获取哑音包数目
	void SetMutePackNum ( u8 byMutePackNum ){ m_byMutePackNum = byMutePackNum;}
	u8 GetMutePackNum ( void ) { return m_byMutePackNum;}

	//设置获取哑音包发送间隔
	void SetMutePackInterval ( u8 byMuteInterval ){ m_byMutePackSendInterval = byMuteInterval;}
	u8 GetMutePackInterval ( void ) { return m_byMutePackSendInterval;}

	//发哑音包相关
	void AddSendMutePack(const TSendMutePack& tSendMutePack);
	void DelSendMutePackByConfIdx(u8 byConfIdx,u8 byMtid = 0);//删除所属会议mtid对应的项，mtid为0删除所属会议的所有项
	BOOL32 IsAudioType(u8 byMediaType);//媒体类型是否为音频
	void PrintSendMuteTable();
    
public:
    u32  m_nInitUdpHandle;
    u32	 m_dwMcuNode;
    u32	 m_dwMcuIId;
    u32  m_dwMcuNodeB;
    u32  m_dwMcuIIdB;
	...
	u8     m_byWIFrameSeqInt;   // 等待关键帧时包序号间隔	

    BOOL32 m_bCancelWaitIFrameNoUni;
	signed char     m_bySeqNumRangeLow;   //seqnum的波动值下限
	signed char     m_bySeqNumRangeUp;    //seqnum的波动值上限
	signed char     m_bySensSeqNum;       //seqnum的敏感值（超出上下限则加敏感值）
	...
	u8                m_byRtcpBackTimerInterval;//发rtcp反馈包的间隔，单位s

	TSendMutePack*    m_ptSendMutePackTable/*[MAX_CONFIDX*MAXNUM_CONF_MT]*/;//要给目的ip和port发哑音包的表。
	u16               m_wNeedSendMutePackNum;//需要给发哑音包的条目个数
...
private:
	...
	PinHoleInfo m_atPinHoleTable[MAX_SWITCH_CHANNEL];   //终端打洞信息表

	u8 m_byMutePackNum;							//发送哑音包包数
	u8 m_byMutePackSendInterval;			    //发送哑音包时间间隔(ms)
	//需要对g7221.c音频格式码流内容奇偶对换保存表
	u16		m_wReverseG7221cPort[MAXnum_ReverseG7221cPort];
};
...
private:    
    ...
    static BOOL32 IsH261KeyFrame( u8 *pBuf);
...
BOOL32 GetMuteFrameData(u8 byMediaType, u32 &dwFrameSize, u8 *pData,u8 bySoundChannelNum = 1);
