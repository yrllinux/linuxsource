...
//	m_nNonStandConfInfoDataLen36 = 0;
    ...
    memset(m_abyMtIpV6, 0, sizeof(m_abyMtIpV6));
    m_byMtIpType = IP_NONE; 
...
#ifndef WIN32
            //IPV6支持
            if ( m_tIncomingCallParams.GetCallingAddr().GetIPAddr_Type() == TYPE_IPV6 )
            {
				//  [5/12/2012 pengguofeng]按小长度拷贝
				// 注意: a::a和a:0000:0000:a可能比较不一致
				u8 byLen = min(sizeof(m_abyMtIpV6), strlen((s8*)m_tIncomingCallParams.GetCallingIp_Ipv6()) + 1);
				LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[IpV6:h_call_incoming]copy length:%d\n", byLen);
                memcpy( m_abyMtIpV6, m_tIncomingCallParams.GetCallingIp_Ipv6(),	byLen);
				m_abyMtIpV6[byLen-1] = 0;

                u32 dwIndex = 0;
                if ( g_cMtAdpApp.IsIpV4Mt( (s8*)m_abyMtIpV6, dwIndex ) )
                {
                    m_dwMtIpAddr = g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV4IP();
                }
                m_byMtIpType = IP_V6;
            }
            else
...
void CMtAdpInst::ProcIncomingCall(BOOL32 bLowerMcuCallingIn)
{
	...
	//终端别名
	tMtAddr.m_AliasType = (u8)mtAliasTypeTransportAddress;
	// 当h_call_incoming里参数Ipv4填0时要加下面这个保护 [pengguofeng 5/31/2012]
	u32 dwSendIpv4 = m_tIncomingCallParams.GetCallingIp();
	if ( dwSendIpv4 == 0)
	{
		dwSendIpv4 = m_dwMtIpAddr;
	}
	LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[ProcIncomingCall]send2Mcu with Ipv4:%x and m_dwMtIpAddr:%x IncomingIPv4:%x\n",
		dwSendIpv4, m_dwMtIpAddr, m_tIncomingCallParams.GetCallingIp());
	tMtAddr.m_tTransportAddr.SetIpAddr( /*ntohl*/(dwSendIpv4/*m_tIncomingCallParams.GetCallingIp()*/) );
...
#else
                u32 dwIpV4 = 0;
                if ( tChan.GetRtp().GetIPAddr_Type() == TYPE_IPV6 )
                {
                    u32 dwIndex = 0;                   
                    if ( g_cMtAdpApp.IsIpV4Mt( (s8*)tChan.GetRtp().GetIPAddr_Ipv6(), dwIndex) )
                    {
                        dwIpV4 = g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV4IP();
                    }
                }

                m_atChannel[nChanIdx].tLogicChan.m_tRcvMediaChannel.SetIpAddr( ntohl( dwIpV4 ) );
#endif
...
#else
            u32 dwIpV4 = 0;
            if ( tNetAddr.GetIPAddr_Type() == TYPE_IPV6)
            {
                u32 dwIndex = 0;
                
                if (g_cMtAdpApp.IsIpV4Mt( (s8*)tNetAddr.GetIPAddr_Ipv6(), dwIndex ))
                {
                    dwIpV4 = g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV4IP();
                }
            }
...
#else
				m_atChannel[nChanIdx].tLogicChan.m_tRcvMediaCtrlChannel.SetIpAddr( dwIpV4 );
#endif
...
#else
				m_atChannel[nChanIdx].tLogicChan.m_tSndMediaCtrlChannel.SetIpAddr( dwIpV4 );
#endif
...
void CMtAdpInst::ProcMcuMtInviteMtReq( CMessage * const  pcMsg )
{
...
#else
	if (!g_cMtAdpApp.m_bGkAddrSet || 0 == g_cMtAdpApp.m_tGKAddr.addr.v4.ip)
#endif
	{
	...
	else if( !g_cMtAdpApp.m_tH323Config.IsManualRAS() && g_cMtAdpApp.m_bGkAddrSet ) 
	{
		...
		tCall.SetCallingAddr_Ipv6( g_cMtAdpApp.m_abyMtadpIpV6, g_cMtAdpApp.m_wQ931Port );
#endif
...
void CMtAdpInst::CallProceed( cmTransportAddress *ptTransportAddr, s32 nApprovedBandwidth, s32 nIrrFrequency )
{
	...
	{		
		...
		else
		{
...
#else
            if (ptTransportAddr->type == cmTransportTypeIPv6)
            {
                u8 abyIpV6Str[IPV6_STR_LEN] = { 0 };
                RvAddressIpv6 callsignaladdrv6;
                memcpy(&callsignaladdrv6.ip, &ptTransportAddr->addr.v6.ip, sizeof(callsignaladdrv6.ip) );
                Rv2AddressIpv6GetString(&callsignaladdrv6, sizeof(abyIpV6Str), (s8*)abyIpV6Str);
                memcpy(/*g_cMtAdpApp.m_abyMtadpIpV6*/m_abyMtIpV6, abyIpV6Str, sizeof(m_abyMtIpV6/*g_cMtAdpApp.m_abyMtadpIpV6*/));
                u32 dwIndex = 0;
                if ( g_cMtAdpApp.IsIpV4Mt( (s8*)abyIpV6Str, dwIndex ) )
                {
                    m_dwMtIpAddr = g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV4IP();
                }
                m_byMtIpType = IP_V6;
            ...
            {
                m_dwMtIpAddr = ptTransportAddr->addr.v4.ip;
                m_byMtIpType = IP_V4;
            }
#endif
...
void CMtAdpInst::CallOutByTransportAddr( BOOL32 bIsIgnoreVcIpCheck /*= FALSE*/ )
{
    ...
    //IPV6支持
    u32 dwIndex = 0;
    BOOL32 bIsIpV6Mt = FALSE;
    if ( g_cMtAdpApp.IsIpV6Mt( ntohl(m_dwMtIpAddr), dwIndex ) )
    {
        bIsIpV6Mt = TRUE;
        tCall.SetCalledAddr_Ipv6( (u8*)g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV6IP(), m_wMtPort);
		u8 byLen = min(strlen(g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV6IP()), sizeof(m_abyMtIpV6));
		LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[CallOutByTransportAddr:IpV6]copy length:%d\n", byLen);
        memcpy( m_abyMtIpV6, g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV6IP(), byLen/*sizeof(m_abyMtIpV6)*/);
        m_byMtIpType = IP_V6;
    }
    else
    {
        tCall.SetCalledAddr( /*ntohl*/(m_dwMtIpAddr), m_wMtPort/*MCU_Q931_PORT*/ );
        m_byMtIpType = IP_V4;
    }
    //tCall.SetCalledAddr( m_dwMtIpAddr, m_wMtPort/*MCU_Q931_PORT*/ );
	LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[[CallOutByTransportAddr]called addr;Type:%d V4:%x V6:%s port:%d\n", tCall.GetCalledAddr().GetCallAddrType(),
		tCall.GetCalledAddr().GetIPAddr(), tCall.GetCalledAddr().GetIPAddr_Ipv6(), tCall.GetCalledAddr().GetIPPort());
#endif
    ...
    {
        tCall.SetCallingAddr( /*ntohl*/(g_cMtAdpApp.m_dwMtAdpIpAddr), g_cMtAdpApp.m_wQ931Port );
    }
	
	LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[[CallOutByTransportAddr]calling addr;Type:%d V4:%x V6:%s port:%d\n", tCall.GetCallingAddr().GetCallAddrType(),
		tCall.GetCallingAddr().GetIPAddr(), tCall.GetCallingAddr().GetIPAddr_Ipv6(), tCall.GetCallingAddr().GetIPPort());
#endif    
...
#else
    u32 dwRtcpIpV4 = tLogicChan.m_tSndMediaCtrlChannel.GetIpAddr();
    u32 dwIndex = 0;
    u8* pbyRtcpIpV6;
    if (g_cMtAdpApp.IsIpV6Mtadp(dwRtcpIpV4, dwIndex))
    {
        pbyRtcpIpV6 = (u8*)g_cMtAdpApp.m_atMtMapTable[dwIndex].GetV6IP();
		LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[ProcMcuMtOpenLogicChannelReq]IpV6 :%s\n", pbyRtcpIpV6);
        tChanParam.m_tRtcp.SetNetAddress_Ipv6( pbyRtcpIpV6, tLogicChan.m_tSndMediaCtrlChannel.GetPort() );
		tChanParam.m_tRtp.SetNetAddress_Ipv6( pbyRtcpIpV6, tLogicChan.m_tRcvMediaChannel.GetPort() );
    }
    else
    {
        tChanParam.m_tRtcp.SetNetAddress( htonl(tLogicChan.m_tSndMediaCtrlChannel.GetIpAddr()), tLogicChan.m_tSndMediaCtrlChannel.GetPort() );
...
#else
        u32 dwIndex = 0;
        if ( g_cMtAdpApp.IsIpV6Mtadp( tLogicChan.m_tRcvMediaCtrlChannel.GetIpAddr(), dwIndex ) &&
             m_byMtIpType == IP_V6 )
        {
            u8 abyIpV6[IPV6_STR_LEN] = {0};
			u8 byLen = min(sizeof(abyIpV6), strlen(g_cMtAdpApp.m_atMtadpMapTable[dwIndex].GetV6IP()));
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[ProcMcuMtOpenLogicChannelRsp]copy length:%d\n", byLen);
            memcpy(abyIpV6, g_cMtAdpApp.m_atMtadpMapTable[dwIndex].GetV6IP(), byLen/*sizeof(abyIpV6)*/);
            tChanParam.m_tRtcp.SetNetAddress_Ipv6( abyIpV6, tLogicChan.m_tRcvMediaCtrlChannel.GetPort() );
        }
        else
        {
            tChanParam.m_tRtcp.SetNetAddress( htonl(tLogicChan.m_tRcvMediaCtrlChannel.GetIpAddr()), tLogicChan.m_tRcvMediaCtrlChannel.GetPort() );
...
#else
					if( 0 == g_cMtAdpApp.m_tGKAddr.addr.v4.ip || 
#endif
						0 == wCallRate || 					             
...
#else
						if( g_cMtAdpApp.m_tGKAddr.addr.v4.ip == 0 || wCallRate == 0 ||
#endif
	...
	case VRS_TERMINAL_PROG_NTF:				//放像进度上报，消息体：当前进度时间（u32）+总进度时间（u32）
		{
			...
			if (m_byRecType == TRecChnnlStatus::TYPE_RECORD)
			{
				cMsg.SetEventId(REC_MCU_RECORDPROG_NOTIF);//消息体为TEqp+TRecProg
			}
			else if (m_byRecType == TRecChnnlStatus::TYPE_PLAY)
			{
...
u8 CMtAdpInst::GetRecStateByVrsState(u8 byVrsState, u8 byRecType)
{
	...
	//录像状态：0 未录像，1 正在录像，2 暂停录像
	//放像状态：0 未放像，1 正在播放，2 暂停播放
	switch (byVrsState)
	{
	case 1:
		{
...
=============================================================================*/
BOOL32 MtAdpStart( TMtIpMapTab* tMtIpMapTab, TMtAdpConnectParam *ptConnectParam, 
				   TMtAdp *ptMtadp,                    
				   ...
				   TMtAdpConnectParam *ptConnectParam2 /*= NULL*/)
{
	if (NULL == ptConnectParam || NULL == ptMtadp
		|| tMtIpMapTab == NULL	//新接口必要参数 [5/12/2012 pengguofeng]
		|| tCriIpMapTab == NULL
		|| tGKIpMapTab == NULL
		)
	{
...
//     #endif
    ...
    //读取IPV6地址映射表
    g_cMtAdpApp.SetMtIpMapTable(&tMtIpMapTab->m_atIpMap[0]);  
    g_cMtAdpApp.SetGkIpMapTable(&tGKIpMapTab->m_atIpMap[0]);  
    g_cMtAdpApp.SetMtadpIpMapTable(&tCriIpMapTab->m_atIpMap[0]);  

	if ( pchV6Ip != NULL)
	{
		u8 byV6IpLen = strlen(pchV6Ip);
		u8 byLen = min(sizeof(g_cMtAdpApp.m_abyMtadpIpV6), byV6IpLen/*strlen(pchV6Ip)*/);
		printf("[MtAdpStart]copy Ip length:%d pchV6Ip:%s swScopeId:%d\n", byLen, pchV6Ip, swScopeId);
		LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[MtAdpStart]copy Ip length:%d swScopeId:%d\n", byLen, swScopeId);
		memcpy(g_cMtAdpApp.m_abyMtadpIpV6, pchV6Ip, byLen);
		g_cMtAdpApp.m_swScopeId = swScopeId;
	}
    ...
    printf("[MtAdpStart] bDoubleLink.%d IPA(%u.%u.%u.%u) IPB(%u.%u.%u.%u), Port RAS= %d, Q931= %d, byMaxCallNum= %d.\n", 
                            g_cMtAdpApp.m_bDoubleLink, QUADADDR(dwMcuAIp), QUADADDR(dwMcuBIp),
                            g_cMtAdpApp.m_wRasPort, g_cMtAdpApp.m_wQ931Port, g_cMtAdpApp.m_byMaxCallNum);
	//enlarge the stack size to accomodate more levels of function calls on real-address systems.
	/*s32 nRet = */g_cMtAdpApp.CreateApp( "MtAdp", AID_MCU_MTADP, APPPRI_MTADP, 5000, 2<<20);
...
#else
	u32 dwIp = g_cMtAdpApp.m_tGKAddr.addr.v4.ip;
#endif
...
#else
	StaticLog("m_tGKAddr:%08x\n",			  g_cMtAdpApp.m_tGKAddr.addr.v4.ip);
#endif
