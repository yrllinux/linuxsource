...
#else
	tMtAdp.SetVersion(DEVVER_MTADPV6);
#endif
	tMtAdp.SetMacAddr(g_cMtAdpApp.m_abyCRIMacAddr);

	CServMsg cServMsg;
	cServMsg.SetMsgBody( (u8 *)&tMtAdp, sizeof(tMtAdp) );//143 -> 149	

#ifndef WIN32
	// 判断IpV6地址是否存在 [pengguofeng 5/18/2012]
	u8 abyStdIpV6[sizeof(g_cMtAdpApp.m_abyMtadpIpV6)];
	memset(abyStdIpV6, 0, sizeof(abyStdIpV6));

	if ( memcmp(abyStdIpV6, g_cMtAdpApp.m_abyMtadpIpV6, sizeof(abyStdIpV6)) != 0 )
	{
		//IPV6地址
		cServMsg.CatMsgBody( g_cMtAdpApp.m_abyMtadpIpV6, sizeof(g_cMtAdpApp.m_abyMtadpIpV6) );
	}
	
	MAPrint(LOG_LVL_DETAIL, MID_MCULIB_MTADP, "[DaemonProcRegisterMcu]MsgLen:%d IpV6 addr:%s scopeId:%d\n",
		cServMsg.GetMsgBodyLen(), g_cMtAdpApp.m_abyMtadpIpV6, g_cMtAdpApp.m_swScopeId);
#endif
...
#else
       u32 dwIndex = 0;
	   LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[DaemonProcMcuRegisterRsp]Mcu Response Gk Ip:0x%x\n", tRsp.GetGkIp());
	   if ( tRsp.GetGkIp() != 0)
	   {
		   if ( g_cMtAdpApp.IsIpV6Gk( tRsp.GetGkIp(), dwIndex ) )
		   {
			   RvAddressIpv6 callsignaladdrv6;  
			   //#ifndef WIN32
			   Rv2AddressIpv6SetString( (s8*)g_cMtAdpApp.m_atGkMapTable[dwIndex].GetV6IP(), &callsignaladdrv6 );
			   //#endif
			   memcpy(g_cMtAdpApp.m_tGKAddr.addr.v6.ip, callsignaladdrv6.ip, sizeof( callsignaladdrv6.ip ));
			   g_cMtAdpApp.m_tGKAddr.type = cmTransportTypeIPv6;
		   }
		   else
		   {
			   g_cMtAdpApp.m_tGKAddr.addr.v4.ip   = htonl(tRsp.GetGkIp());
			   g_cMtAdpApp.m_tGKAddr.type = cmTransportTypeIP;
		   }
	   }
	   else
	   {
		   //如果MCU回过来GkIP是0，则V4Ip也设成0
		   memset(&g_cMtAdpApp.m_tGKAddr, 0, sizeof(g_cMtAdpApp.m_tGKAddr));
// 		   g_cMtAdpApp.m_tGKAddr.addr.v4.ip   = 0;
	   }
#endif
...
#else
		0 != g_cMtAdpApp.m_tGKAddr.addr.v4.ip &&
#endif
...
//  [pengguofeng 6/1/2012]
// 	g_cMtAdpApp.m_tH323Config.Clear();
	...
	if( NULL == g_cMtAdpApp.m_hApp )
	{
...
#else
		if( g_cMtAdpApp.m_tGKAddr.addr.v4.ip != 0 )
		{
			g_cMtAdpApp.m_tH323Config.SetDefGKIP( g_cMtAdpApp.m_tGKAddr.addr.v4.ip );
		}
#endif
		...
		s8 achIpV4[17] = {0};
		s8 achIpV6[64] = {0};
		s32 nSetIp = 0; //有无设置Ip[V4/V6]地址
		s8 achIpv6File[64] = {0};
		s32 nScopeId = 0;
		// 取消限制，对所有系统均有效：conf下的ipv6confg [pengguofeng 6/4/2012]
// #ifdef _LINUX_
		sprintf(achIpv6File, "%s/ipv6config.ini", DIR_CONFIG);
		printf("[H323Init] get IpAddr from %s\n", achIpv6File);
		//1 get bSet
		GetRegKeyInt(achIpv6File, "IpAddr", "bSet", 0, &nSetIp);
		printf("[H323Init] Is Ip Addr Set in ipv6config.ini:%d\n", nSetIp);
#ifdef WIN32
		nSetIp = 0; // [pengguofeng 6/11/2012]win32不需要读配置文件
#endif
		if ( nSetIp != 0)
		{
			GetRegKeyString(achIpv6File, "IpAddr", "Ipv4", "0", achIpV4, 17);
			GetRegKeyString(achIpv6File, "IpAddr", "Ipv6", "invalid", achIpV6, 64);
			GetRegKeyInt(achIpv6File, "IpAddr", "ScopeId", 0, &nScopeId);
			printf("Ipv4:%s IpV6:%s ScopeId:%d\n", achIpV4, achIpV6, nScopeId);
		}
// #endif
#if defined(_8KE_) || defined(_8KH_) || defined(_8KI_)
		u32 dwIp_N_4H323 = 0;//INET_ADDR(achIpV4)/*g_cMcuAgent.GetMpcIp()*/;
		if ( nSetIp == 0 )
		{
			dwIp_N_4H323 = g_cMcuAgent.GetMpcIp();//有可能顺序不符合传给协议栈
		}
		else
		{
			dwIp_N_4H323 = INET_ADDR(achIpV4);
		}
		// 填0是防止linux初始化崩溃[协议栈问题,目前已经解决] [pengguofeng 5/28/2012]
		if ( dwIp_N_4H323 != 0)
		{
			printf("[H323Init]set ipv4:0x%x\n", dwIp_N_4H323);
...
// 		g_cMtAdpApp.m_tH323Config.SetStackIP( dwIp_N_4H323 ); //need net sequence
#else
        //支持IPV6，V4与V6地址设置协议栈
		// 目前设置为0是因为同时填V4和V6的话，H323初始化失败 [pengguofeng 5/24/2012]
		if ( nSetIp == 0)
		{
			g_cMtAdpApp.m_tH323Config.SetStackIP(g_cMtAdpApp.m_dwMtAdpIpAddr);
		}
		else
		{
			printf("[H323Init]set ipv4:%s\n", achIpV4);
			g_cMtAdpApp.m_tH323Config.SetStackIP(INET_ADDR(achIpV4));
		}
#endif	

#ifndef WIN32
		// Set Ipv6 Addr [pengguofeng 6/4/2012]
		if ( nSetIp == 0)
		{
			if ( g_cMtAdpApp.m_abyMtadpIpV6 != NULL && strlen((s8*)g_cMtAdpApp.m_abyMtadpIpV6) != 0)
			{
				g_cMtAdpApp.m_tH323Config.SetStackIP_Ipv6( g_cMtAdpApp.m_abyMtadpIpV6, g_cMtAdpApp.m_swScopeId );
			}
		}
		else
		{
			if ( strncmp(achIpV6, "invalid", strlen("invalid")) != 0)
			{
				g_cMtAdpApp.m_tH323Config.SetStackIP_Ipv6( (u8 *)achIpV6, nScopeId );
				// 同时保存的变量也需要改成此值 [pengguofeng 6/14/2012]
				memset(g_cMtAdpApp.m_abyMtadpIpV6, 0, sizeof(g_cMtAdpApp.m_abyMtadpIpV6));
				u8 byIpv6Len = strlen(achIpV6);
				u8 byMinLen = min(sizeof(g_cMtAdpApp.m_abyMtadpIpV6), byIpv6Len);
				memcpy(g_cMtAdpApp.m_abyMtadpIpV6, achIpV6, byMinLen);
				g_cMtAdpApp.m_swScopeId = nScopeId;
			}
		}
		// 参数打印 [pengguofeng 6/4/2012]
		printf("[h323Init]v4IP:%x v6IP:%s\n", g_cMtAdpApp.m_tH323Config.GetStackIP(),
			g_cMtAdpApp.m_tH323Config.GetStackIP_Ipv6());
		printf("[h323Init]m_swScopeId.%d, scopeid.%d\n", g_cMtAdpApp.m_swScopeId,
			g_cMtAdpApp.m_tH323Config.GetStackIP_Ipv6_ScopeId());
#endif

		printf("MaxCalls.%d, MaxChannels.%d, EndpointType.%d, MasterSlaveManual.%d\n",
			g_cMtAdpApp.m_tH323Config.GetMaxCalls(),g_cMtAdpApp.m_tH323Config.GetMaxChannels(),
			g_cMtAdpApp.m_tH323Config.GetEndpointType(),
			g_cMtAdpApp.m_tH323Config.IsMasterSlaveManual());
		printf("CapabilitiesManual.%d, IsManualRAS.%d,IsManualReg.%d, H225CallingPort.%d\n",
			g_cMtAdpApp.m_tH323Config.IsCapabilitiesManual(),
			g_cMtAdpApp.m_tH323Config.IsManualRAS(),
			g_cMtAdpApp.m_tH323Config.IsManualReg(),
			g_cMtAdpApp.m_tH323Config.GetH225CallingPort());
		printf("AliasType.%d, AliasName.%s,ProductIdSize.%d FirstPort.%d, LastPort.%d\n",
			g_cMtAdpApp.m_tH323Config.m_atAlias[0].GetAliasType(),
			g_cMtAdpApp.m_tH323Config.m_atAlias[0].GetAliasName(),
			g_cMtAdpApp.m_tH323Config.GetProductIdSize(),
			g_cMtAdpApp.m_tH323Config.GetFirstPort(),
			g_cMtAdpApp.m_tH323Config.GetLastPort());
		printf("VersionIdSize.%d, m_abyProductId.%s, m_nProductIdSize.%d, m_abyVersionId.%s, m_nVerIdSize.%d\n",
			g_cMtAdpApp.m_tH323Config.GetVersionIdSize(),
			g_cMtAdpApp.m_tH323Config.m_abyProductId,
			g_cMtAdpApp.m_tH323Config.m_nProductIdSize,
			g_cMtAdpApp.m_tH323Config.m_abyVersionId,
			g_cMtAdpApp.m_tH323Config.m_nVerIdSize);
		
        phApp = kdvInitH323Adapter(NULL/*g_achStackCfgFile*/, &g_cMtAdpApp.m_tH323Config);
        // zbq [07/04/2007]
        g_cMtAdpApp.m_bMtAdpInited = TRUE;
	...
	//将GK地址写入协议栈
	// 未初始化0，gk地址会被填成CCCC...,即204.204... [pengguofeng 5/24/2012]
// #ifdef WIN32
// 	memset(&g_cMtAdpApp.m_tGKAddr, 0, sizeof(g_cMtAdpApp.m_tGKAddr));
// #endif
...
{
#ifdef WIN32
	u32 dwIpv4 = g_cMtAdpApp.m_tGKAddr.ip;
#else
	u32 dwIpv4 = g_cMtAdpApp.m_tGKAddr.addr.v4.ip;
#endif
    ...
    if(0 != memcmp(&g_cMtAdpApp.m_tGKAddrOld, &tGKAddr, sizeof(cmTransportAddress)))
    {
        ...
        {
            g_cMtAdpApp.m_bGkAddrSet = (0 != dwIpv4/*g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/) ? TRUE : FALSE;            
            ...
            MAPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[SetGKRasAddress] kdvSetGKRASAddress g_hApp.0x%x, GkAddr(%u.%u.%u.%u:%d), GKNetAddrType.%d\n", 
                g_cMtAdpApp.m_hApp, QUADADDR(dwIpv4/*g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/), g_cMtAdpApp.m_tGKAddr.port, g_cMtAdpApp.m_tGKAddr.type );
        }
        else
        {
            MAPrint(LOG_LVL_ERROR, MID_MCULIB_MTADP, "[SetGKRasAddress]: return failure in SetGKRasAddress(), g_hApp.0x%x, GkAddr(%u.%u.%u.%u:%u)!\n",
                g_cMtAdpApp.m_hApp, QUADADDR(dwIpv4/*g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/), g_cMtAdpApp.m_tGKAddr.port );
            return FALSE;
        }		
    }    
    else
...
BOOL32 CMtAdpData::SetMtIpMapTable( TIpMap* ptMtMapTable )
{
    if (ptMtMapTable == NULL)
    {
        StaticLog("[SetMtIpMapTable]pointer param is null!\n");
        return FALSE;
    }

    memcpy( m_atMtMapTable, ptMtMapTable, sizeof(m_atMtMapTable));
    return TRUE;
}  

BOOL32 CMtAdpData::SetGkIpMapTable(TIpMap* ptGkMapTable )
{
    if ( ptGkMapTable == NULL)
    {
        StaticLog("[SetGkIpMapTable]pointer param is null!\n");
        return FALSE;
    }
    
    memcpy( m_atGkMapTable, ptGkMapTable, sizeof(m_atGkMapTable));
    return TRUE;
}
 
BOOL32 CMtAdpData::SetMtadpIpMapTable(TIpMap* ptMtadpMapTable )
{
    {
        if ( ptMtadpMapTable == NULL)
        {
            StaticLog("[SetMtadpIpMapTable]pointer param is null!\n");
            return FALSE;
        }
        
        memcpy( m_atMtadpMapTable, ptMtadpMapTable, sizeof(m_atMtadpMapTable));
        return TRUE;
    }
}

/*
 *	判断MT是不是IPv6,从m_atMtMapTable读取
 */
BOOL32 CMtAdpData::IsIpV6Mt( const u32 dwIpV4, u32 &dwIndex)
{
    BOOL32 bRet = FALSE;
    for ( u32 dwMapIndex = 0; dwMapIndex < MAXNUM_CONF_MT; dwMapIndex++ )
    {
        if ( m_atMtMapTable[dwMapIndex].GetV4IP() == dwIpV4 )
        {
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "input:0x%x and find MT success[%d]\n",
				dwIpV4, dwMapIndex);
            dwIndex = dwMapIndex;
            bRet = TRUE;
            break;
...
/*
 *	判断GK是不是IPv6,从m_atGkMapTable读取
 */
BOOL32 CMtAdpData::IsIpV6Gk( const u32 dwIpV4, u32 &dwIndex)
{
    BOOL32 bRet = FALSE;
    for ( u32 dwMapIndex = 0; dwMapIndex < 10; dwMapIndex++ )
    {
        if ( m_atGkMapTable[dwMapIndex].GetV4IP() == dwIpV4 )
        {
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "input:0x%x and find GK success[%d]\n",
				dwIpV4, dwMapIndex);
            dwIndex = dwMapIndex;
            bRet = TRUE;
            break;
...
/*
 *	判断MtAdp是不是IPv6,从m_atGkMapTable读取
 */
BOOL32 CMtAdpData::IsIpV6Mtadp( const u32 dwIpV4, u32 &dwIndex)
{
    BOOL32 bRet = FALSE;
    for ( u32 dwMapIndex = 0; dwMapIndex < MAXNUM_DRI; dwMapIndex++ )
    {
        if ( m_atMtadpMapTable[dwMapIndex].GetV4IP() == dwIpV4 )
        {
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "input:0x%x and find MtAdp success[%d]\n",
				dwIpV4, dwMapIndex);
            dwIndex = dwMapIndex;
            bRet = TRUE;
            break;
...
/*
 *	判断Mt是不是IPv4,从m_atGkMapTable读取
 */
BOOL32 CMtAdpData::IsIpV4Mt( /*const*/ s8* pbyIpV6, u32 &dwIndex)
{
    BOOL32 bRet = FALSE;
    for ( u32 dwMapIndex = 0; dwMapIndex < MAXNUM_DRI; dwMapIndex++ )
    {
        s8* pbyIpV6Str = m_atMtMapTable[dwMapIndex].GetV6IP();
        if ( 0 == memcmp( (void*)pbyIpV6, (void*)pbyIpV6Str,  strlen(pbyIpV6) + 1 ) )
        {
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "input:%s and find Mt success[%d]\n",
				pbyIpV6, dwMapIndex);
            dwIndex = dwMapIndex;
            bRet = TRUE;
            break;
...
/*
 *	判断GK是不是IPv4,从m_atGkMapTable读取
 */
BOOL32 CMtAdpData::IsIpV4Gk( /*const*/ s8* pbyIpV6, u32 &dwIndex)
{
    BOOL32 bRet = FALSE;
    for ( u32 dwMapIndex = 0; dwMapIndex < 10; dwMapIndex++ )
    {
        u8* pbyIpV6Str = (u8*)m_atGkMapTable[dwMapIndex].GetV6IP();
        if ( 0 == memcmp( (void*)pbyIpV6, (void*)pbyIpV6Str,  strlen(pbyIpV6) + 1 ) )
        {
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "input:%s and find GK success[%d]\n",
				pbyIpV6, dwMapIndex);
            dwIndex = dwMapIndex;
            bRet = TRUE;
            break;
...
/*
 *	判断MtAdp是不是IPv4,从m_atMtadpMapTable读取
 */
BOOL32 CMtAdpData::IsIpV4Mtadp( /*const*/ s8* pbyIpV6, u32 &dwIndex)
{
    BOOL32 bRet = FALSE;
    for ( u32 dwMapIndex = 0; dwMapIndex < 10; dwMapIndex++ )
    {
        u8* pbyIpV6Str = (u8*)m_atMtadpMapTable[dwMapIndex].GetV6IP();
        if ( 0 == memcmp( (void*)pbyIpV6, (void*)pbyIpV6Str,  strlen(pbyIpV6) + 1 ) )
        {
			LogPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "input:%s and find MtAdp success[%d]\n",
				pbyIpV6, dwMapIndex);
            dwIndex = dwMapIndex;
            bRet = TRUE;
            break;
						...
						u16          wBufLen     )
{
	...
	LogPrint(LOG_LVL_DETAIL,MID_MCULIB_MTADP,"[EvMMcuMsg] haChan.%p\n",haChan);
