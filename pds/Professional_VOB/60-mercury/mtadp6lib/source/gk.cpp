...
BOOL32 CMtAdpInst::ProcSendARQ( TMtAlias &tSrcAddress, TMtAlias &tDstAddress, u16 wBandwidth )
{
	...
	if( !g_cMtAdpApp.m_bGkAddrSet && 
        ( ( g_cMtAdpApp.m_tGKAddr.type == cmTransportTypeIP && g_cMtAdpApp.m_tGKAddr.addr.v4.ip ) ||
          ( g_cMtAdpApp.m_tGKAddr.type == cmTransportTypeIPv6 && memcmp(g_cMtAdpApp.m_tGKAddr.addr.v6.ip, abyIpV6, sizeof(abyIpV6) ) )
        )
      )
#else
	if( !g_cMtAdpApp.m_bGkAddrSet && g_cMtAdpApp.m_tGKAddr.ip != 0 )
#endif
	{
		MAPrint(LOG_LVL_KEYSTATUS, MID_MCULIB_MTADP, "[ProcSendARQ]New Ip%d[0 presents IPV4] in\n", g_cMtAdpApp.m_tGKAddr.type);
	...
	if( FALSE == g_cMtAdpApp.m_bMasterMtAdp )
	{
		...
		tGKID.length = ( /*UINT16*/ u16) g_cMtAdpApp.m_tGKID.GetIDlength();
		...
		tEPID.length = ( /*UINT16*/ u16) g_cMtAdpApp.m_tEPID.GetIDlength();
...
#ifndef WIN32
						  QUADADDR(tSrcAlias.transport.addr.v4.ip), tSrcAlias.transport.port );
#else
...
#ifndef WIN32
						  QUADADDR(tDestAlias.transport.addr.v4.ip), tDestAlias.transport.port );
#else
...
#else
	if( g_cMtAdpApp.m_tGKAddr.type == cmTransportTypeIP && g_cMtAdpApp.m_tGKAddr.addr.v4.ip == 0 )
#endif
	{
...
#else
		QUADADDR(g_cMtAdpApp.m_tGKAddr.addr.v4.ip) );
#endif
		return FALSE;
	}
	if( !g_cMtAdpApp.m_bGkAddrSet )
...
#else
	tRasAddr.addr.v4.ip   = g_cMtAdpApp.m_dwMtAdpIpAddr;
#endif
...
#else
        tCallAddr.addr.v4.ip   = g_cMtAdpApp.m_atMtadpAddr[byIdx].GetNetSeqIpAddr();
#endif
...
#else
        u32 dwCallIp = ntohl(tCallAddr.addr.v4.ip);
#endif
...
BOOL32 CMtAdpInst::DaemonProcSendURQ( CMessage * const pcMsg )
{
...
#else
        byConfIdx, QUADADDR(g_cMtAdpApp.m_tGKAddr.addr.v4.ip), g_cMtAdpApp.m_bGkRegistered );
#endif
...
#else
			byConfIdx, QUADADDR(g_cMtAdpApp.m_tGKAddr.addr.v4.ip));
#endif
		return FALSE;
	}		
...
#else
        u32 dwCappIp = ntohl(tCallAddr.addr.v4.ip);
#endif
        if( cmRASSetParam( hsRas, cmRASTrStageRequest, cmRASParamCallSignalAddress,
	...
	if( cmRASSetParam( hsRas, cmRASTrStageRequest, cmRASParamFeatureSignalling, 0, 18, NULL ) < 0 )
    {
        MAPrint( LOG_LVL_ERROR, MID_MCULIB_MTADP, "[DaemonProcSendURQ]set cmRASParamFeatureSignalling error\n");
    }
	...
	case h_ras_ACF:
		{
            ...
            if ( ptAcfInfo->m_tCalledAddr.GetIPAddr_Type() == TYPE_IPV6 )
            {
                Rv2AddressIpv6SetString( (s8*)ptAcfInfo->GetCalledAddr().GetIPAddr_Ipv6(), &callsignaladdrv6 );
                memcpy( &tTransportAddr.addr.v6.ip, &callsignaladdrv6.ip, sizeof(callsignaladdrv6.ip) );
                m_byMtIpType = IP_V6;
                tTransportAddr.type = cmTransportTypeIPv6;
            }
            else
            {
				m_byMtIpType = IP_V4;
                tTransportAddr.addr.v4.ip = ptAcfInfo->m_tCalledAddr.GetIPAddr();
                tTransportAddr.type = cmTransportTypeIP;
            }
#endif
            ...
            /*lint -save -e507*/
			/*HAPPCALL *pTmp = const_cast<HAPPCALL *>(haCall);*/
...
{	
	...
	if ( g_cMtAdpApp.m_bMasterMtAdp /*&& 0 != g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/ && g_cMtAdpApp.m_bGkRegistered &&
#ifdef WIN32
...
void CMtAdpInst::DaemonProcConnectToGK()
{
	...
	if ( 0 == dwIp/*g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/ )
    {
        return;
    }
    u32 dwNode = (u32)OspConnectTcpNode( dwIp/*g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/, PORT_GK );
    if ( OspIsValidTcpNode(dwNode) )
    {
        ...
        StaticLog("[MtAdp] Connect to GK %u.%u.%u.%u:%u succeed, start register\n", 
                                                     QUADADDR(dwIp/*g_cMtAdpApp.m_tGKAddr.addr.v4.ip*/), PORT_GK );
    }
    else
    {
...
#else
		QUADADDR(g_cMtAdpApp.m_tGKAddr.addr.v4.ip), g_cMtAdpApp.m_dwGKNode );
#endif		
    }
    else
    {
...
//     }
...
#else
		if ( g_cMtAdpApp.m_tGKAddr.addr.v4.ip != htonl(tRASInfo.GetGKIp()) )
#endif
    {
...
#else
	tCallAddr.addr.v4.ip = htonl(tRASInfo.GetRRQIp());
#endif
    ...
    if ( 0 != tRASInfo.GetEPID()->GetIDlength() ) 
    {
        ...
        tEPID.length = ( /*UINT16*/ u16) tRASInfo.GetEPID()->GetIDlength();
    ...
    else
    {
        MAPrint(LOG_LVL_ERROR, MID_MCULIB_MTADP, "[NPlusUnregGKCmd] fake epid length.%d, and URQ failed !\n", 
                                      ( /*UINT16*/u16 )tRASInfo.GetEPID()->GetIDlength() );
        g_cMtAdpApp.m_bURQForNPlus = FALSE;
        return;
    }
