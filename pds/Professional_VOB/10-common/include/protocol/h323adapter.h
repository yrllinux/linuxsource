...
enum H225CallMsgType 
{
	...
	//end
    //jacky add h_call_alerting event for KDV1000
...
enum  PayloadType 
{
    emPayloadTypeBegin = 0,
		...
		//add for ZTE MCU nostandrd [20130703]
        t_ZTE_MCU,//video 且未设入同时能力中
		//end [20130703]
...
// 			m_byProfile = 64;
	...
	BOOL operator <= (const tagH264VideoCap& tH264VideoCap)
	{
		...
		//add by shigubin 20120615 防止不同厂商间因为MBPS微小差异引起开不起通道，比如说polycom的1080P 30fps的MaxMBPS是490，而我们自己的是486
// 		else if( ( ( tH264VideoCap.GetMaxMBPS()/tH264VideoCap.GetMaxFS()*tH264VideoCap.GetMaxFS() ) < tH264VideoCap.GetMaxMBPS() ) 
// 			&& ( m_nMaxMBPS <= ( tH264VideoCap.GetMaxMBPS()/tH264VideoCap.GetMaxFS() + 1)*tH264VideoCap.GetMaxFS() )
// 			&& (m_nMaxFS <= tH264VideoCap.GetMaxFS()) 
// 			&& ( m_byProfile | tH264VideoCap.GetProfileValue() ) == tH264VideoCap.GetProfileValue() )
// 		{
// 			return TRUE;
// 		}

		//add by shigubin 20121210 polycom来开1080P 30的时候会以 1088*1920的分辨率来开，这时候的MaxMBPS为490，而我们的1080P 30Fps为486导致我们认为是比较的时候用1080P 60Fps比较
		else if( GetResolution( emHD1080 ) && m_nMaxFS > 8100 && m_nMaxFS <= 8192 )
		{	
			s32 SimilarMaxMBPS = ( m_nMaxMBPS/( (1920*1088)/256 ) ) * ( (1920*1080)/256 );
			if( SimilarMaxMBPS <= tH264VideoCap.GetMaxMBPS() && m_nMaxFS <= tH264VideoCap.GetMaxFS() 
				&& ( m_byProfile | tH264VideoCap.GetProfileValue() ) == tH264VideoCap.GetProfileValue() )
				return TRUE;
		}
...
typedef struct tagCallParam 
{
	...
	void SetUserInfo(const u8* pchUserInfo, u8 byLen)
	{
        ...
        memset(m_abyUserInfo, 0, LEN_USERINFO_MAX);
...
private:
    ...
    //MAX_DESCAP_NUM    :组：单个音频，第1路视频，第2路视频
    //MAX_SIMUCAP_NUM   :路
    //MAX_ALTCAP_NUM    :每一路里的能力
...
public:
	...
	BOOL SetCapDesc(u8 byCapIndex, u8 bySimuIndex, u8 byAltIndex, u16 wPayload, u8 byH264SubId = 0)
	{
		...
		if (byCapIndex >= MAX_DESCAP_NUM || bySimuIndex >= MAX_SIMUCAP_NUM || byAltIndex >= MAX_ALTCAP_NUM )
		{
			return FALSE;
		}
	...
	//add by shigubin for standard mp4 (在能力表里面强制添加一个能力)
    void SetCapDescId(u8 byCapIndex, u8 bySimuIndex, u8 byAltIndex, u16 capId)
	{
		if( m_aaawCapDesc[byCapIndex][bySimuIndex][byAltIndex] != 0xFFFF )
			return ;
		else
			m_aaawCapDesc[byCapIndex][bySimuIndex][byAltIndex] = capId;
	}
	//add end
	...
	BOOL IsH264CapALessEqualB( const TH264VideoCap & tH264A, const TH264VideoCap & tH264B )
	{
		...
		if ( tH264A.GetMaxStaticMBPs() != 0 )
		{
			...
			nAMaxMBPS = (s32)(1.0 / ( ( ( 4.0 / dAMaxFS ) / dAMaxMBPS ) + ( ( ( dAMaxFS - 4.0 ) / dAMaxFS ) / dAMaxStaticMBPS ) ));
		}
		else
		{
			nAMaxMBPS = tH264A.GetMaxMBPS();
		}
		...
		if ( tH264B.GetMaxStaticMBPs() != 0 )
		{
			...
			nBMaxMBPS = (s32)(1.0 / ( ( ( 4.0 / dBMaxFS ) / dBMaxMBPS ) + ( ( ( dBMaxFS - 4.0 ) / dBMaxFS ) / dBMaxStaticMBPS ) ));
		}
		else
		{
			nBMaxMBPS = tH264B.GetMaxMBPS();
		}

		if( ( nAMaxMBPS <= nBMaxMBPS ) && ( tH264A.GetMaxFS() <= tH264B.GetMaxFS() ) )
		{
			return TRUE;
		}
		else
...
typedef enum
{
	...
	KDVP_NSM_GETMCULIST_REQ = 5,                                //网真向GK请求会议列表
	KDVP_NSM_GETMCULIST_RSP,
	
	KDVP_NSM_GETALLLIST_REQ,                                //网真向gk请求终端+会议列表
	KDVP_NSM_GETALLLIST_RSP,
...
返   回   值: 网络序的IP地址
====================================================================*/
u32	 kdvGetLocalCallIPAddress(IN HCALL hsCall);


//add by shigubin for 
/*====================================================================
函	 数   名: SetCapSetWithTrueMp4
功	      能: 让capset具有真实的mp4能力，并跟老版本的非标mp4兼容
算 法 实  现: 
引用全局变量: 无
输入参数说明: ptCapSet    - TCapSet指针
输出参数说明：ptCapSet    - 转换好的ptCapSet
返   回   值: void
====================================================================*/
u8 AddExistCapToCapSet( PayloadType payload,  TCapSet* ptCapSet );
//add end
...
API int kdvDnsRequest(s8* pchName, UINT32 dwDnsServer, UINT32 dwUserID, 
...
#define ADP_IW_PRODUCT_ID     (const char *)"InterworkingGateway"
#define ADP_IW_VERSION_ID     (const char *)"40.00.01.04.111209"
