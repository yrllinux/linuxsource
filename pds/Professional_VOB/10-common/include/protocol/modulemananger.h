...
protected:
	...
	virtual BOOL PreDaemonMessageProc( CMessage *const ptMsg )	{ return FALSE; }		    //daemon消息预处理函数，需要重载
	
protected:
	virtual BOOL32 ObjectInit( u32 wParam, u32 lParam ) = 0;									//object初始化函数，需要重载
	virtual void ObjectExit( void );															//object退出函数，需要重载
	
	virtual BOOL PreMessageProc( CMessage *const ptMsg )	{ return FALSE; }			    //消息预处理函数，需要重载
...
{
	s8 m_achTitle[MAX_TITLE_LEN+1];		//模块title名字			
	TXEVENTINFO* m_ptXmlTable;			//XML编码表
	u32 wParam;
	u32 lParam;			//用户自定义
	SEMHANDLE Sem;
