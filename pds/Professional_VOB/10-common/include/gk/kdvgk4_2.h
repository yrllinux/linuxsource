...
//H460服务器pinhole APPID
#define AID_GK_H460PINHOLE             (AID_GKS_BGN + 7)	//add by yj for 460server[20130325]
...
// GK网络地址类型定义
#define GK_NET_NONE 0x00
#define GK_NET_IPV4 0x01
#define GK_NET_IPV6 0x02

#if defined(GK_CFLAG_IPV6)
  #undef  GK_NET_TYPE
  #define GK_NET_TYPE   GK_NET_IPV6
#else
  #undef  GK_NET_TYPE
  #define GK_NET_TYPE   GK_NET_IPV4
#endif


//add by yj for ipv6
#if (GK_NET_TYPE & GK_NET_IPV6)
...
//#define TEPE_SCTP           RV_NET_SCTP         //表示同事支持IPv4、IPv6地址
#endif
//end
...
#define LEN_1							(u16)1
#define MAX_SERVERCALL_NUM				(u16)512/*128*/			//服务器最大支持呼叫数
...
//add by yj 
#define GKDB_KDV_PATH               (s8*)"/usr/etc/gk/GKKDVDB"								//自定义数据库路径
#define GKCONFIGDB_KDV_PATH         (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB"					//自定义数据库路径

#define USERDB_KDV_PATH_NAME        (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB/UserDB.db"
#define BASECONFIGDB_KDV_PATH_NAME  (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB/BaseConfigDB.db"
#define BANDWIDTHDB_KDV_PATH_NAME   (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB/BandwidthDB.db"
#define IPRANGEDB_KDV_PATH_NAME     (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB/IPRangeDB.db"
#define ALIASDB_KDV_PATH_NAME       (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB/EndpointAliasDB.db"
#define ENTITYDB_KDV_PATH_NAME      (s8*)"/usr/etc/gk/GKKDVDB/GKKDVConfigDB/EntityDB.db"          //[12/26/2007 zhangzhihua]add 
//end
...
//add by yj 
#define GKDB_KDV_PATH               (s8*)"GKKDVDB"											//自定义数据库路径
#define GKCONFIGDB_KDV_PATH         (s8*)"GKKDVDB/GKKDVConfigDB"									//自定义数据库路径

#define USERDB_KDV_PATH_NAME        (s8*)"./GKKDVDB/GKKDVConfigDB/UserDB.db"
#define BASECONFIGDB_KDV_PATH_NAME  (s8*)"./GKKDVDB/GKKDVConfigDB/BaseConfigDB.db"
#define BANDWIDTHDB_KDV_PATH_NAME   (s8*)"./GKKDVDB/GKKDVConfigDB/BandwidthDB.db"
#define IPRANGEDB_KDV_PATH_NAME     (s8*)"./GKKDVDB/GKKDVConfigDB/IPRangeDB.db"
#define ALIASDB_KDV_PATH_NAME       (s8*)"./GKKDVDB/GKKDVConfigDB/EndpointAliasDB.db"
#define ENTITYDB_KDV_PATH_NAME      (s8*)"./GKKDVDB/GKKDVConfigDB/EntityDB.db"          //[12/26/2007 zhangzhihua]add 
//end
...
OSPEVENT( EV_RAS_RECV_SCI_TIMEOUT, GKRASOSPEVENT_START + 49 );//add by yj for SCI timeout [20130604]	
...
OSPEVENT( EV_PRINT_ALLDB,                   GKCTRLOSPEVENT_START + 148);//打印所有数据库信息[20120827]
...
public:
        ...
        if ( bGetFromStack == FALSE )
        {
                ...
                || aliasType == cmAliasTypeGatekeeperID)
            {
				...
				if ( bUtf8 )
				{
					...
					m_wAliasNameLen = utlUTF82Uni( (s8*)aszUTF8Str, (RvUint8*)m_achAliasName );
                ...
                m_wAliasNameLen = utlChr2Bmp( chrStr, (RvUint8*)m_achAliasName );
	...
	//end [20130712]
    {
            ...
            || aliasType == cmAliasTypeGatekeeperID)
        {
			...
			{
				u8 aszUTF8Str[MAX_ALIASNAME_UTF8_LEN+1] = {0};
				wNameLen = utlUni2UTF8((RvUint16*)m_achAliasName, wAliasNameLen, (s8*)aszUTF8Str);
				if ( wNameLen < wAliasNameLen )
					return 0;
				strncpy(pszName, (char *)aszUTF8Str, wNameLen); 
			}
			else
			{
			//end [20130712]
				s8 chrStr[MAX_ALIASNAME_LEN+1] = {0};
				utlBmp2Chr( chrStr, (RvUint8*)m_achAliasName, wAliasNameLen );
...
// 	}
	...
	void SetNetAddr_Ipv6(u8* dwIPAddr_ipv6, u16 wPort/*, s16 swScopeId */)
	{
...
//		m_swScopeId = swScopeId;
	...
	void SetIPAddr_Ipv6(u8* dwIPAddr_ipv6)
    {
        memcpy(m_dwIP_ipv6, dwIPAddr_ipv6, sizeof(m_dwIP_ipv6));
		m_byIPType = TYPE_IPV6;
	}
...
typedef struct tagNeighborGKInfo
{
	...
	//add by yj for ipv6
	u8          m_byIPType;                                 //支持的 IP地址类型
	u8          m_dwNeighborGKIP_ipv6[IPV6_NAME_LEN];	    //运行IP(ipv6)，非网络序;
//	s16         m_swScopeId;                                //获取或设置 IPv6 地址范围标识符
    //end
#endif
    ...
    tagNeighborGKInfo()
    {
		...
		//add by yj for ipv6
		m_byIPType = TYPE_IPV4;   
		memset(m_dwNeighborGKIP_ipv6, 0, sizeof(m_dwNeighborGKIP_ipv6));
//		m_swScopeId = -1;
        //end
#endif
...
#if (GK_NET_TYPE & GK_NET_IPV6)
	//add by yj for ipv6
//	u8          m_byIPAddrFromType;                                 //支持的 IP地址类型
	u8          m_byIPType;                                 //支持的 IP地址类型
	u8          m_dwIPAddrFrom_ipv6[IPV6_NAME_LEN];			        //IP(ipv6)，非网络序;
//	u8          m_byIPAddrToType;                                 //支持的 IP地址类型
	u8          m_dwIPAddrTo_ipv6[IPV6_NAME_LEN];			        //IP(ipv6)，非网络序;
    //end 
#endif
...
public:
    tagAddrRng()
    {
		...
		//add by yj for ipv6
        m_byIPType = TYPE_IPV4;
		memset(m_dwIPAddrFrom_ipv6, 0, sizeof(m_dwIPAddrFrom_ipv6));
		memset(m_dwIPAddrTo_ipv6, 0, sizeof(m_dwIPAddrTo_ipv6));
		//end
#endif
    }
	...
	void SetIPAddrFrom_Ipv6( u8* dwIPAddr_ipv6 )
    {
		memcpy(m_dwIPAddrFrom_ipv6, dwIPAddr_ipv6, sizeof(m_dwIPAddrFrom_ipv6));
		m_byIPType = TYPE_IPV6;
    }
    u8* GetIPAddrFrom_Ipv6()
    {
        return m_dwIPAddrFrom_ipv6;
    }
    
    void SetIPAddrTo_Ipv6( u8* dwIPAddr_ipv6 )
    {
		memcpy(m_dwIPAddrTo_ipv6, dwIPAddr_ipv6, sizeof(m_dwIPAddrTo_ipv6));
		m_byIPType = TYPE_IPV6;
    }
    u8* GetIPAddrTo_Ipv6()
    {
        return m_dwIPAddrTo_ipv6;
    }
	//end
#endif
...
#if (GK_NET_TYPE & GK_NET_IPV6)
	//add by yj for ipv6
	u8    m_dwIPFrom_ipv6[IPV6_NAME_LEN];			    //IP(ipv6)，非网络序;
//	u8    m_dwIPFromMask_ipv6[IPV6_NAME_LEN];           //IP掩码(ipv6),非网络序;

	u8    m_dwIPTo_ipv6[IPV6_NAME_LEN];			        //IP(ipv6)，非网络序;
//    u8    m_dwIPToMask_ipv6[IPV6_NAME_LEN];             //IP掩码(ipv6),非网络序;
	//end
#endif
...
#if (GK_NET_TYPE & GK_NET_IPV6)
	//add by yj for ipv6
	void SetIPFrom_Ipv6( u8* dwIPAddr_ipv6/*, u8* dwIPMask_ipv6*/ )
    {
		memcpy(m_dwIPFrom_ipv6, dwIPAddr_ipv6, sizeof(m_dwIPFrom_ipv6));
 //       memcpy(m_dwIPFromMask_ipv6, dwIPMask_ipv6, sizeof(m_dwIPFromMask_ipv6));
    }
    u8* GetIPFrom_Ipv6()
    {
        return m_dwIPFrom_ipv6;
    }
//     u8* GetIPFromMask_Ipv6()
//     {
//         return m_dwIPFromMask_ipv6;
//     }
    
    void SetIPTo_Ipv6( u8* dwIPAddr_ipv6/* u8* dwIPMask_ipv6 */)
    {
		memcpy(m_dwIPTo_ipv6, dwIPAddr_ipv6, sizeof(m_dwIPTo_ipv6));
//		memcpy(m_dwIPToMask_ipv6, dwIPMask_ipv6, sizeof(m_dwIPToMask_ipv6));
    }
    u8* GetIPTo_Ipv6()
    {
        return m_dwIPTo_ipv6;
    }
//     u8* GetIPToMask_Ipv6()
//     {
//         return m_dwIPToMask_ipv6;
//     }
	//end
#endif
...
#if (GK_NET_TYPE & GK_NET_IPV6)
		//add by yj for ipv6
		memset(m_dwIPFrom_ipv6, 0, sizeof(m_dwIPFrom_ipv6));
//		memset(m_dwIPFromMask_ipv6, 0, sizeof(m_dwIPFromMask_ipv6));
		memset(m_dwIPTo_ipv6, 0, sizeof(m_dwIPTo_ipv6));
//		memset(m_dwIPToMask_ipv6, 0, sizeof(m_dwIPToMask_ipv6));
		//end
#endif
...
#if (GK_NET_TYPE & GK_NET_IPV6)
	//add by yj for ipv6
	u8              m_byIPType;            //IP地址类型
	u8              m_dwServerIP_ipv6[IPV6_NAME_LEN];//Radius服务器IPv6地址(非网络序)
	//end
#endif 
...
public:
    tagRadiusConfigInfo()
    {
		...
		//add by yj for ipv6
        m_byIPType = TYPE_IPV4;
		memcpy(m_dwServerIP_ipv6, "::1", IPV6_NAME_LEN);
		//end
#endif
...
#if (GK_NET_TYPE & GK_NET_IPV6)
	//add by yj for ipv6
    void SetServerIP_Ipv6(u8* dwServerIP_ipv6)
    {
        memcpy(m_dwServerIP_ipv6, dwServerIP_ipv6,sizeof(m_dwServerIP_ipv6));
		m_byIPType = TYPE_IPV6;
    }
    u8* GetServerIP_Ipv6()
    {
        return m_dwServerIP_ipv6;
    }
	//end
#endif
...
//add by yj for DB
#define USERDB_KEY_LEN				MAX_USERNAME_LEN 
#define USERDB_DATA_LEN				sizeof(BOOL)+sizeof(u16)+MAX_PASSWORD_LEN

#define BASECONFIGDB_KEY_LEN		sizeof(TConfigBaseGKDBData)
#define BASECONFIGDB_DATA_LEN		128

#define BANDWIDTHDB_KEY_LEN			sizeof(TIPRange)
#define BANDWIDTHDB_DATA_LEN		sizeof(TBandwidthCtrlData)

#define IPRANGEDB_KEY_LEN			sizeof(TAddrRng)	
#define IPRANGEDB_DATA_LEN			sizeof(BOOL)

#define ENTITYDB_KEY_LEN			sizeof(TEntityAuthKey)
#define ENTITYDB_DATA_LEN			0

#define ALIASDBB_KEY_LEN			MAX_ALIASNAME_LEN + sizeof(cmEndpointType) +1
#define ALIASDB_DATA_LEN			MAX_PASSWORD_LEN + sizeof(u8)

#define FILE_ConfigDB_LEN			256//参考最大邻居GK数
#define FILE_DB_LEN					64
//end
