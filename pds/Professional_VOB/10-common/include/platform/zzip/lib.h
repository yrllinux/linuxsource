...
struct zzip_dir_hdr
{
    ...
    uint8_t     d_compr;        /* the compression type, 0 = store, 8 = zip_inflate */
...
int      __zzip_try_open (void* mem, zzip_size_t size, int filemode,
                          zzip_strings_t* ext, zzip_plugin_io_t io);

ZZIP_DIR * 
zzip_dir_fdopen(void *mem, zzip_size_t size, zzip_error_t * errcode_p);
ZZIP_DIR* 
zzip_dir_fdopen_ext_io(void* mem, zzip_size_t size, zzip_error_t * errorcode_p,
                       zzip_strings_t* ext, const zzip_plugin_io_t io);
#endif
...
#ifdef _VXWORKS_
/* ¸´ÖÆ×Ö·û´® */
static char *_strdup( const char *strSource )
{
	size_t nlen = strlen(strSource) + 1;
	char* strDest = (char*)malloc(nlen);
	memset(strDest, 0, nlen);
